...
import { fuseWithRegime } from './ensemble-fuser'
...
export class TradingEngine {
  ...
  async getAiDecision(symbol: string): Promise<AiDecision> {
    ...
    const depth = this.depth.get(symbol)  // P-009: wire L2 depth → depth_imbalance + microprice_dev
    const raw   = await this.brain.decide(symbol, quote, ind, depth)
    // L1.F — Ensemble regime-aware confidence fusion.
    // Scales raw confidence by a regime × EMA-alignment multiplier before
    // calibration so the calibrator receives an already regime-adjusted signal.
    const regime = this.observer?.getRecent(symbol, 1)?.[0]?.regime ?? null
    const fused  = fuseWithRegime(raw.confidence, raw.bias, regime, ind)
    // Calibration choke point — every consumer sees the calibrated confidence.
    return { ...raw, confidence: this.calibration.calibrate(fused) }
  }
