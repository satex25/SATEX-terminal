...
- **P-098: cold-boot intro rework — the 3.2s splash is now frame one of a 4-frame branded boot sequence.** New `BootIntroSequence` overlay chains the existing `SplashIntro` (1a, mono-scoped via `--bb-*` variable overrides) into three new full-bleed plates from the operator's `Intro Rework.dc.html` mockup: 1b **Masthead** (meridian shaft, 128px tracking reveal 0.04em→0.5em over 2.3s, credits, dissolve exit), 1c **Tape Head** (head-switch band, live 25fps timecode `00:MM:SS:FF`, `satex-splash-*` scan/letter reuse, steps(28) typewriter, one 0.4s wobble, CRT-collapse exit), 1d **System Plate** (registration marks, drawn rules, 252px character-rise wordmark, right-aligned metadata table, 7s mount-progress rule, hairline-wipe exit). Each of 1b–1d plays a full 7.0s boot — no skip — then holds on a pulsing PRESS ANY KEY; plain keys advance, while bare modifiers and chords (⌘⇧K kill-arm, ⌘K palette) fall through untouched and the overlay never calls preventDefault — the kill chord stays reachable from every UI state (P-044 lineage). Transition table + formatters live headless in `src/renderer/lib/intro-sequence.ts` (the `rail-layout.ts` pattern) with degenerate-input guards (P-040 class); `prefers-reduced-motion` collapses the whole sequence to the splash's existing fast fade; UTC clock/date and a session label derived from the UTC hour (TOKYO/LONDON/NEW YORK — not the mockup's hardcoded LONDON, per Directive 0.1) tick at 200ms; renderer-only, zero IPC or perimeter contact — the terminal keeps warming underneath and mounts the instant the final wipe lands. Tests: `intro-sequence.test.ts` 18/18 (full walk = 3 keypresses + 23.4s scripted timers, no-skip rule, hold-on-enter, chord filtering, formatter clamps). Gates (2026-07-12, in-sandbox): typecheck node+web exit 0 · eslint scoped to all 8 touched files exit 0 (full lint > 45s sandbox ceiling — CI arbiter) · vitest segmented exact-cover 127 files / 1,683 tests / 0 fail (12 invocations) · knip not sandbox-runnable (P-097, CI arbiter) with a clean static export audit. Deferred with ledger follow-ups: Playwright `boot-sequence.spec.ts` (Electron unlaunchable in-sandbox; unverified test code violates 0.4), Settings toggle for scanlines, single-source version stamp. See P-098; bridge-corruption scar hit twice during the build — see P-099.
