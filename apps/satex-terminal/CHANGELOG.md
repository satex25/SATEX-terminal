...
- **P-159: RS-1.4 parity oracle in Rust — the harness that every later parity claim is measured with (unshipped sibling crate; zero TypeScript-side contact).** `crates/satex-parity` grows from the RS-0.2 stub into the Rust half of the oracle: a hand-written JSON value model (JS `JSON.parse` overflow semantics, UTF-16 code-unit key ordering, JS shortest-round-trip number spelling — three behaviours `serde_json` does not offer together), the golden loader and its unilateral format laws, the ordinal-aligned structural differ, drift reports (human summary ported byte-for-byte from `verify.ts`, plus the Appendix A.4 JSONL the TS side never implemented), corpus-tape custody that distrusts its own header per P-143, and a dependency-free CLI. 6,079 lines, **one** runtime dependency (`sha2`, with its D-012 justification row). The RS-1.7 49-class mutation matrix is re-expressed natively and **all 49 classes hold** — 36 caught and correctly named, 13 container-only perturbations correctly ignored. Falsifiability was proved rather than assumed: deliberately breaking the scalar comparison fails 14 of 49 classes, and three further tests point the judge at fabricated always-clean / always-divergent / wrong-field oracles and demand it reject all three (P-097). Verified end-to-end on the real 1,396-record RS-1.3 golden: identical inputs → CLEAN exit 0; a single-ULP nudge to one field of one record → exit 1 naming `record 82 · seq 82 · tick 1908 · L1 · autonomy.decision · payload.confidence`. Harness runs in fixture mode only — full-engine replay awaits the Phase 2 cores, and baseline comparators are RS-1.5. Rust six-gate bar: fmt · clippy · test (138 passed / 0 failed) · doc · machete · deny, all exit 0. No TypeScript source changed.
- **P-156: characterization coverage for the 8 renderer leaf presentational components (test-only; subjects byte-unchanged).** `StatPill`, `Ring`, `SessionPill`, `Icon`, `PanelHead`, `RailSlot`, `Modal` and `Dropdown` — the operator-facing legibility primitives (P3) — shipped with zero tests. 8 jsdom characterization suites now pin their DOM output using the dependency-free `createRoot`+`act` harness proven in the P-129/P-133 chart program (no test-library dependency added). `Modal` and `Dropdown` additionally assert listener and portal removal on unmount (Escape and mousedown inert post-unmount), regression-pinning the PR #6 / §2.5.7 leak class on the two live interactive primitives. Authored 2026-07-24 but left untracked and unexecuted; rescued, verified, and renumbered from the colliding P-137 claim. Gates: `src/renderer/components/` 10 files / 51 tests 0 failed; full suite 175 files / 2,234 tests 0 failed; typecheck · lint · knip all exit 0.
- **P-153/P-154/P-155: RS-1.3 golden-capture driver — the oracle can capture (dev/CI tooling under `scripts/oracle/`; zero engine-source contact).** The TS side of the parity oracle: boots the real `TradingEngine` headless with no Electron runtime, replays a corpus session on a virtual clock, and emits golden JSONL per plan Appendix A.3. Seven modules — golden record contract (canonical serialisation, id normaliser, stream hash), hermetic sandbox (scratch filesystem, pinned environment, closed network with an audit trail), electron stub, corpus reader plus deterministic synthesizer, corpus→scratch-DB importer with real-database assertions, the capture driver, and the archive writer — with 99 tests. Runs under vitest so the double-run determinism proof executes in CI on every push with no workflow change, and so the fake timers arrive via vitest's bundled `@sinonjs/fake-timers` rather than a new dependency. **Two captures of the rescued 35,658-row corpus tape produce byte-identical goldens**, closing plan risk R3 for the captured state surface. It also surfaced **P-155**: unseeded `Math.random()` in the depth ladder reaches roughly a quarter of every AI confidence score via `depth_imbalance`/`microprice_dev`, so the decision stream is not yet reproducible — open for an operator ruling, and pinned by a test that fails when it is fixed. No engine source changed.

- **P-135: RS-UP-1 adopted — the Rust engine rewrite ultraplan enters execution (workspace-level; zero app-code contact).** The 7-layer plan to re-implement the Electron main process as a Rust core under Tauri 2 moved from repo root to `docs/plans/2026-07-22-satex-rs-rewrite-ultraplan.md` (v1.0.1 — baseline re-measured at adoption: `master @ d5d0922`, ledger claim renumbered P-130→P-135), decision register D-001–D-016 operator-ratified (D-017 open pending RS-1.2 corpus sizing), Scar-Tissue Port Ledger seeded at `docs/plans/rs-scar-ledger.md` (Appendix B.4). The Rust workspace grows additively at `apps/satex-engine-rs` (RS-0.2); this Electron terminal stays the shipping engine, untouched, until the audited M5 flip — validation-strangler strategy per plan D-001. Toolchain installed at adoption: rustc/cargo 1.97.1 `stable-x86_64-pc-windows-msvc`. Gates: docs-only change; typecheck+lint at pre-commit, four-gate bar CI-arbitrated on the PR.
...
- **P-158: the ARCHITECTURE.md §4 baseline is now measured rather than asserted, and CI fails when it drifts (repo tooling + CI; zero app-code contact).** Two defects in one documentation line. **(1) The provenance was a template constant.** `update-baseline.sh` emitted the literal `+ edits (working tree)` on every run without ever consulting `git status`, so a baseline measured on a clean checkout still claimed uncommitted edits — an assertion about the tree in the very file that documents AGENTS.md's "report real results, never assert them" rule. `write` now asks git and records `(clean tree)` or `+ uncommitted edits (working tree)` truthfully. **(2) Nothing enforced the counts.** They drifted silently: `27aa7dc`'s subject records "1753/134 was 10 days stale", caught by eye rather than by a gate, and drift recurred within two days. CI's existing "Vitest unit suite" step now also emits `--reporter=json`, and a new `scripts/check-baseline.sh` reads those counts and **exits 1** on mismatch, printing the exact fix command — the suite is never run twice. New `scripts/baseline.mjs` is the single source of truth for the line's format, shared by writer and checker so the two cannot disagree by a character and deadlock CI. `update-baseline.sh` keeps its `<files> <tests>` CLI but drops the 4-way shard that scraped vitest's human-readable summary (that text is not a contract; the JSON report is). Implementation trap documented at the call site: vitest's `numTotalTestSuites` counts `describe` blocks, not files (one probe file reported 4) — the file count is `testResults.length`. Verified both directions: against the real report `✓ baseline is current` exit 0, and with counts perturbed to 2251/176 exit **1** with the correct diff — a check only ever observed passing proves nothing. Known residual, recorded not papered over: `main-protection`'s linear-history rule means `--rebase` rewrites the sha this line records, so the sha stays an honest record of what was measured, not a durable pointer; enforcing sha reachability was rejected because master is protected and a post-merge failure could only be cleared by another PR. Gates: typecheck 0 · lint 0 · knip 0 · **2238 tests / 175 files** green (Node-ABI worktree). Full record: `Vault/00-Audit/PROBLEM-LEDGER.md` P-158.
...
- **P-157: the depth ladder is seeded — the AI decision stream now replays exactly, closing P-155 and unblocking Oracle L1 decision parity.** `DepthFeedService.jitterFor()` churned the L2 ladder with four unseeded `Math.random()` calls per tick, and the top of that ladder becomes `depth_imbalance` (weight 0.15) and `microprice_dev` (0.10) in `brain.ts` via `getAiDecision` — so roughly **a quarter of the feature weight behind every AI confidence could not be drawn twice**, on the live path as well as under replay. Measured before the fix: identical symbol, tick index and virtual timestamp, confidence `0.3520162749933342` vs `0.36683881944775815`. Fix per the operator's P-155 Option 2 ruling: `DepthFeedService` takes an optional `seed?: number` second constructor argument and builds a `mulberry32` from it — the same seam `MarketSimulator` has used since RS-0.7 — and `trading-engine.ts` passes `env.rngSeed ?? undefined`, **unconditionally** rather than only under `!useAlpaca`, because Alpaca's free tier serves no L2 and this synthesizer feeds the brain on the live path too. Omitting the seed still falls back to `randomSeed()`, so live trading keeps its entropy and determinism stays opt-in via `SATEX_RNG_SEED`. The ladder is seeded `(seed ^ 0x9E3779B9) >>> 0` rather than the raw seed: two `mulberry32`s seeded alike emit the same sequence, and the simulator receives that same `env.rngSeed`, so an unsalted ladder would march in step with the GBM price walk and let `depth_imbalance` encode the price move twice. **Proof:** the inverted P-155 tripwire — a test deliberately written to fail the day it was fixed — fired; the divergence did not relocate to another record kind (which the pin's docblock flagged as evidence of a *second* nondeterminism), it vanished. Two full captures of the 35,622-tick corpus tape with the autonomous trader running are now byte-identical, and the archived golden went from **146 records / 0 decisions to 1,396 records / 14 decisions** (`348ec08f…` → `cfdb9e06…`). The tripwire was kept and inverted rather than deleted as its docblock suggested — L1 is the stratum Appendix A.3 gives no tolerance, so trading a failing assertion for no assertion would have left the most consequential records uncovered; it now carries a vacuous-pass guard requiring decisions to exist before their equality counts. Also fixed alongside: the golden archive writer hardcoded `CAPTURE_DEFAULTS.speed` (10) into the manifest while the corpus capture runs at speed 1, so the manifest would have misdescribed the capture that produced its own golden. Perimeter-adjacent (§2.4), landed on an explicit operator ruling. Gates (2026-07-26, operator hardware, Node-ABI worktree): typecheck node+web exit 0 · eslint exit 0 · knip exit 0 · vitest **167 files / 2193 tests / 0 fail**.
