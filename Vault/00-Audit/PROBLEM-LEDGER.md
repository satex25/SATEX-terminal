...
### P-108 · `Sparkline.tsx` spreads an unbounded array into `Math.min`/`Math.max` — the one renderer left off the repo's hardened `extent.ts` single-pass pattern (P-093 class) — LATENT (not live: all call-sites bounded) — OPEN
...
- **Status:** OPEN (latent hardening; off-perimeter; low blast-radius). Claim on the next renderer session.
