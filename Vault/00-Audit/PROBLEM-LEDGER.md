...
### P-114 · Two IPC payload schemas (`CredentialsSetReq`, `AlpacaModeSetReq`) were missing `.strict()` — the file's own stated convention, applied to 17 sibling schemas, was silently not universal — FIXED (prepared, unmerged) — DECIDED, perimeter sign-off required
...
- **Status:** DECIDED — fix prepared and bundled (`mc4/p114-ipc-strict-credentials-alpacamode.bundle`, branch `fix/p114-ipc-strict-credentials-alpacamode`), awaiting operator review + sign-off + PR. Flip SHIPPED after merge, VERIFIED after CI green.
...
- **Status:** SHIPPED to branch, gates green, PR opened — **awaiting operator §0.3 arm before merge** (per the landing directive: prepare, do not merge).
