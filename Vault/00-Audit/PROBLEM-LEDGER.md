...
- **DECISION + rationale:** **Surfaced, not decided.** This is trading-safety perimeter (CONSTITUTION §0.3/§2.4) — the intentional-latch-vs-latent-defect call is the operator's, not an autonomous one. The P-094 coverage PR that found this **pins the current behavior** (the `tactics.test.ts` veto-stickiness test: a winning streak does not lift the veto), so whichever way the operator rules, a deliberate future change turns that test red on purpose. No source touched by this finding.
- **Gates:** N/A — finding only, no code change. Reachability evidence verified against `tactics.ts:88-89` (only push/shift), `:169-179` (refresh branches), `:187-193` (running-max drawdown), and a zero-hit grep for history-reset paths across `src/`.
- **Status:** FINDING (2026-07-22) — OPEN, operator disposition pending (solution 1 vs 2 vs 3). Current behavior regression-pinned in `tactics.test.ts` (P-094 pass). Not a blocker for the P-094 coverage PR.
