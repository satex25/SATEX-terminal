...
### P-098 · Cold-boot intro is a single 3.2s splash — the operator-approved 4-frame branded boot (Intro Rework) was unimplemented — SHIPPED (feat/boot-intro-sequence, 2026-07-12)
...
- **Status:** SHIPPED on local branch `feat/boot-intro-sequence` (committed via the proven /tmp-clone → push-to-mount workflow after `allow_cowork_file_delete` cleared the P-099 lock debris; working tree byte-identical to the commit; index rebuilt via `rm .git/index && git reset`, the 2026-07-10 recovery). No GitHub creds in-sandbox — operator: `git push -u origin feat/boot-intro-sequence` → PR (required check `CI Gates`, rebase/squash) → merge after green. Awaiting CI knip (sandbox can't run it, P-097).
