...
- **DECISION + evidence:** seven modules under `apps/satex-terminal/scripts/oracle/`, **68 new tests**, zero engine edits. `sandbox.ts` (scratch tree + pinned env + closed network), `electron-stub.ts`, `corpus.ts` (reader + deterministic synthesizer), `importer.ts`, `capture.ts` (the driver), `archive.ts`, plus `capture.determinism.test.ts` — the proof. Design record and regeneration procedure: `docs/plans/rs-1.3-golden-driver.md`.
  ...
  - **Scope exclusion, stated rather than tolerated (A.2 requires this).** `depth-feed.ts:89-92` perturbs its ladder with **unseeded `Math.random()`** and `RegimeService` consumes its VPIN. Neither is captured; **depth and regime are outside Oracle L1/L2 until they take a seeded RNG.** Including them today would fail the double-run proof — the tripwire working, not a tolerance to widen.
...
- **OPEN — operator decisions (three):** (1) **Should goldens be committed?** `Vault/Backtests/*` is gitignored, so the archived golden lives only on operator hardware — the same exposure that cost 49 of 50 tapes in P-143. Unlike the corpus (D-017: 4.6–12 GB for 20 sessions, which disqualified git), a golden is **92 KB**; twenty is ~2 MB. Recommend a `!Vault/Backtests/goldens/` negation so a parity claim can cite a committed artifact. (2) Ratify the depth/regime L1/L2 exclusion, or schedule the seeded-RNG seam. (3) RS-1.2 corpus breadth (≥20 sessions, five regime tags) still needs operator recordings — that, not this task, is now the M1 critical path.
