...
- **SOLUTIONS considered:**
  1. *Wrap `serde_json` and diff its `Value` tree* — fastest, and wrong in three specific ways the matrix would catch. `serde_json` rejects `1e999` as a parse error where JS `JSON.parse` yields `Infinity`; Appendix B.3 makes a non-finite float a **caught engine bug** (an `invariant` finding on `payload`), so `float-overflow-to-infinity` would be reported under the wrong category. Its `Map` orders keys by UTF-8 bytes, not the UTF-16 code units `Object.keys().sort()` uses. And `{}` on an `f64` gives `1000000000000000000000` where JS gives `1e+21`. Correcting all three around the library is more code than the parser.
  2. *Hand-write the JSON model* (chosen) — ~900 lines including tests, **zero** runtime dependencies, and each of the three behaviours above is a documented decision with a test rather than a wrapper fighting its library.
  3. *Export the 49 mutations from TypeScript as committed fixtures and assert the Rust oracle against the identical bytes* — genuinely the strongest conformance evidence, and **rejected on scope**: §0.D permits exactly three TypeScript-side changes (RS-1.3 driver, RS-9.3 typegen, RS-0.4 CI) and a fixture exporter is a fourth. RS-L8 says a better idea is a ledger entry, not a diff. The matrix is re-expressed natively instead, against a base golden written out in full so the perturbations stay reproducible.
- **DECISION + evidence:** `crates/satex-parity`, 6,079 lines across 7 modules + CLI + conformance suite. `value` (JSON model, canonical text, JS number spelling, UTF-16 key order) · `record` (envelope + drift vocabulary) · `id` (`IdNormalizer`, hand-rolled `\b`-correct id scan, no regex crate) · `load` (unilateral read: parse + the format's own laws) · `diff` (ordinal-aligned structural comparison) · `report` (human summary ported byte-for-byte + the Appendix A.4 JSONL half, which the TS side never implemented) · `corpus` (tape/index reader that distrusts its own header, per P-143) · `src/bin/satex-parity.rs` (dev/CI CLI, `std::env::args`, no clap).
  - **One dependency, with its D-012 row in `Cargo.toml`:** `sha2` 0.10.9, for the SHA-256 the corpus and golden manifests pin content by. Pinned to 0.10 rather than the live 0.11.0 (re-verified via `cargo search`, RS-L5) because 0.10.9 is already in the local registry cache, keeping gate runs hermetic without network.
  - **Zero TypeScript-side changes** — `git status --short apps/satex-terminal` returns 0 lines. §0.D held.
- **PROOF (measured, not asserted):** six-gate bar, all green on operator hardware — `fmt 0 · clippy 0 · test 0 (138 passed / 0 failed) · doc 0 · machete 0 · deny 0` (`advisories ok, bans ok, licenses ok, sources ok`). Test count was 31 before this work (satex-core only); satex-parity adds 107.
  - **The instrument was proved falsifiable, not merely observed passing.** All 49 classes passed on the first run, which is exactly when a matrix deserves suspicion, so the differ was deliberately sabotaged (`if se != sa` → `if false` in `diff.rs`'s scalar branch) and the suite re-run: **14 of 49 classes failed**, each naming the class and the contract violated. Restored and re-verified. Three further tests point the judge at fabricated verdicts — an always-clean oracle, an always-divergent one, and one that names the wrong field — and demand it reject all three; without them the 13 silent negative controls would prove nothing. P-097 applied to our own measuring instrument.
  - **End-to-end against the real RS-1.3 artifact**, not only synthetic fixtures: `Vault/Backtests/goldens/ses_mrynz0vlkf0x001.golden.jsonl` (1,396 records, 907 KB) against itself → `ORACLE VERDICT: CLEAN — 1396 records, no divergences (byte-identical)`, exit 0. Then **one field of one record nudged by a single ULP** (`payload.confidence` 0.18522048141880315 → …318 on an L1 `autonomy.decision`) → exit 1, `1 divergence(s) over 1396 reference / 1396 candidate records`, naming `record 82 · seq 82 · tick 1908 · L1 · autonomy.decision · payload.confidence`, with the Appendix A.4 JSONL row carrying `golden_sha`/`session`/`subsystem`.
- **PORTED QUIRKS — carried deliberately under RS-L1, each documented at the code:**
  1. **A divergence cap of `0` is a false green.** `diffGoldens` checks `collected.length >= limit` before pushing, so a cap of 0 collects nothing and an empty list reads as CLEAN. The TypeScript has the identical hole. Ported rather than quietly closed (RS-L8); the **CLI refuses `--max-divergences 0` at parse time (exit 2)**, which is the right place for an edge the library must not silently move. Fixing the library itself is a candidate follow-up entry.
  2. **`seq`/`tickIndex` narrowed to `u64`** (Appendix B.1 licenses integer narrowing "where TS semantics are integral in practice"; `renderRecord` already refuses both unless `Number.isInteger(v) && v >= 0`). `ts` is deliberately **not** narrowed — it stays `f64`, because the writer does not require it to be integral and `ts-sub-millisecond-drift` exists to keep it that way.
  3. **Duplicate JSON keys are last-wins**, matching JS and serde. A parser that rejected them would still "catch" `payload-duplicate-json-key`, but as a parse error rather than the value divergence the matrix expects.
- **ENVIRONMENT FINDING (fixed, affects every future crate):** this was the **first external dependency in the Rust workspace**, so nothing had ever exercised the linker. Git Bash ships its own `link.exe` (GNU coreutils) at `C:\Program Files\Git\usr\bin`, which shadows the MSVC linker on PATH, and `LIB` was unset — `cargo check` failed first with a coreutils usage error, then LNK1181. Resolved by putting MSVC Build Tools 14.44.35207 first on PATH and naming the Windows SDK 10.0.26100 import-library directories. **Deliberately not committed as `.cargo/config.toml`** — a hardcoded machine-local path would break the Ubuntu and windows-latest CI jobs, which are configured correctly already. Recorded here because the next agent to add a crate on this hardware will hit it.
- **SCOPE — what RS-1.4 does *not* yet do, by design:** the harness runs in **fixture mode** only. The plan's spec says so: *"as crates mature, harness grows subsystem adapters — early: pure-core fixture mode; later: full-engine replay mode"* — full-engine replay needs the Phase 2+ cores, which do not exist. Baseline comparators against `Vault/Backtests/baselines/brain-*.json` are **RS-1.5**, a separate task, and are not claimed here.
- **Status:** SHIPPED. Six gates green, 138/138 tests. Scar-ledger row P-097 moved `pending → ported` (Appendix B.4). **Not a milestone claim:** M1 exit is an operator ceremony (§0.D), and RS-1.5 + RS-1.6 sit alongside this in Phase 1.
...
- **CODE REVIEW (superpowers:requesting-code-review, `4c4934d..d72dcac`) caught one Critical before merge — `e617b5a`:** `check-baseline.sh` and `update-baseline.sh` were git-tracked `100644` (non-executable). CI execs `check-baseline.sh` directly (`run: ../../scripts/check-baseline.sh …`), and a fresh checkout gets exactly the tracked mode — so the new gate would have failed with "Permission denied" on **every** CI run, not just on real drift, i.e. the "SHIPPED" claim above was false until this fix landed. Local testing hadn't caught it: `core.fileMode=false` on this machine means git never picked up the on-disk `+x` bit at `git add` time. Fixed with `git update-index --chmod=+x` on both scripts (matching `git-unlock.sh`, already tracked `100755`). Also Important: `check-baseline.sh`'s two `read -r ... <<<"$(node …)"` calls didn't propagate node's exit code (a command-substitution failure inside a herestring isn't caught by `set -e`), so a malformed report or a missing "Baseline " line fell through to the stale-diff branch with blank numbers instead of a clear error — both call sites now check the exit status explicitly and exit 2 with a specific message. Re-verified by hand post-fix, all four paths: current (exit 0), malformed report (exit 2, clear message), real drift (exit 1, correct diff+fix hint), missing baseline line (exit 2, clear message) — the last two are new coverage the original PROOF above didn't exercise. Two Minor findings (no `g`-flag guard against duplicate "Baseline " lines; `numTotalTests` now counts skipped tests too, moot while none exist) left as-is per the reviewer's own recommendation.
- **FIRST REAL CATCH — 2026-08-07, PR #87 (`8f6021f`).** Everything above proves the gate *can* fail; this is the first time it failed on drift nobody planted. RS-1.7 (`b046494`) added three suites under `scripts/oracle/` — +131 tests, +3 files — and its commit body deliberately left §4 alone: *"Baseline line left alone; the coordinator refreshes it centrally (P-158)."* That deferral is only safe if something remembers, and something did. CI:
  ```
  ✗ ARCHITECTURE.md baseline is stale
      recorded: 2238 tests / 175 files
      actual:   2369 tests / 178 files
    fix: scripts/update-baseline.sh 178 2369
  ```
  Every step ahead of it — typecheck, lint, knip, the full 2,369-test suite — passed, so the baseline check was the **only** failing step in the job and it failed for exactly the reason it exists. The two Rust jobs were green in parallel, which is what made the red unambiguous.
  - **Restamped from a fresh local measurement, not from the CI message.** Copying `178 2369` out of the failure text would have satisfied the gate while reintroducing the defect it was built to kill — §4 would once again be a number asserted rather than observed. The suite was re-run in the Node-ABI worktree (`vitest --reporter=json` → `success: true · 2369 passed / 2369 · 0 failed · 178 files`) and the counts read from *that* report; they matched the runner's exactly, which is itself the evidence that the two environments agree.
  - **Cost, measured:** one command on one PR. That is the price quoted in the DECISION above, now paid once in anger rather than estimated.
- **Status:** SHIPPED. Gates: typecheck 0 · lint 0 · knip 0 · 2238/2238 tests green on operator hardware (Node-ABI worktree). `check-baseline.sh`/`update-baseline.sh` confirmed executable in the git tree (`100755`) as of `e617b5a` — the gate this ledger entry describes now actually runs on CI, not just locally, and as of `8f6021f` it has blocked a real merge.
...
- **OPERATOR RULING (2026-07-26):** P-155 Option 2 — *seed `DepthFeedService` from the `env.rngSeed` seam `MarketSimulator` already uses*. Options 3 and 4 (ignore/zero depth under replay) stay rejected for the reason P-155 gave: they change decision semantics between live and replay, so a replay would no longer reproduce the session it claims to.
- **SOLUTION SHIPPED:** `DepthFeedService` takes an optional second constructor argument `seed?: number` and builds a `mulberry32` from it, exactly as `MarketSimulator` does; the four `Math.random()` calls became `rng.nextInt()`/`rng.next()`. `trading-engine.ts` passes `env.rngSeed ?? undefined` at construction — **unconditionally**, not only under `!useAlpaca`, because Alpaca's free tier serves no L2 and this synthesizer is what feeds the brain on the live path too. Omitting the seed still falls back to `randomSeed()`, so live trading keeps its entropy and determinism stays opt-in via `SATEX_RNG_SEED`.
  - **Sub-seed, not the raw seed.** The depth stream is seeded `(seed ^ 0x9E3779B9) >>> 0`. Two `mulberry32`s seeded with the same number emit the same sequence, and the simulator receives that same `env.rngSeed`; feeding the ladder the identical stream as the GBM price walk would let `depth_imbalance` encode the price move a second time. One line, documented at the constant.
- **PROOF (measured, not asserted):** the inverted P-155 tripwire — a test written to *fail the day it was fixed* — fired: `firstDivergence` returned `null` where it had been demanding a divergence. Notably the divergence did not **move** to another record kind, which is what the pin's docblock warned would indicate a second, separate nondeterminism; it vanished. Two full captures of the 35,622-tick corpus tape with the autonomous trader running are now byte-identical.
- **THE ARTIFACT CHANGED SHAPE.** Regenerated golden: **146 records → 1,396 records, 0 decision records → 14**; hash `348ec08f…` → `cfdb9e06…`. The manifest records `speed: 1` — the speed actually used for the corpus capture, not `CAPTURE_DEFAULTS.speed` (10), which the writer had hardcoded. Left as-is, the manifest would have misdescribed the capture that produced its own golden, and regenerating from it would not have reproduced those bytes. Same doc-truth class as P-095/P-104/P-148, caught before it shipped rather than after.
- **The pin was inverted, not deleted.** P-155's docblock instructed the fixer to *delete* the tripwire. It is kept and flipped to its positive form instead: L1 is the no-tolerance stratum, and trading a failing assertion for no assertion would have left the most consequential records uncovered. What was evidence of the defect is now the regression guard against its return, with an explicit vacuous-pass guard (decisions must exist before their equality is allowed to mean anything).
- **One pre-existing test changed meaning.** `depth-feed.test.ts` pinned `Math.random` to `0.5` suite-wide, which made the churn delta exactly `0` — the jitter-continuity test got its exactness from that. Seeding makes the pin inert, so continuity is now asserted as it actually behaves (a round trip preserves accumulated drift, moving at most the slots one tick can). The new determinism tests deliberately **restore** the real `Math.random`: under the pin, a determinism test would pass just as well against the unseeded code it exists to forbid.
- **Gates:** typecheck 0 · lint 0 · knip 0 · **167 files / 2193 tests** green on operator hardware (Node-ABI worktree). The ARCHITECTURE.md §4 baseline line is deliberately **not** touched here — PR #84 rewrites that same line; post-merge of both it is 175 files / 2238 tests.
- **Status:** SHIPPED. **Unblocks:** RS-1.4 parity on the decision stream; full closure of plan risk R3 (was split — state/gates reproducible, judgement not). **Still open:** RS-1.7 oracle mutation testing still gates every parity *claim* — this proves the stream is stable, not that the comparator would catch a real difference.

### P-132 · Repo hygiene: machine-local Claude settings untracked-and-unignored, and a superseded adoption runbook still at repo root — SHIPPED 2026-07-23 (`8ab9b1d`); **ledger entry reconstructed retroactively 2026-07-26**
- **Provenance note — read this first:** this entry was **written after the fact** by reading commit `8ab9b1d`, not contemporaneously. It is placed here, at the ledger head, rather than back-dated into the P-131…P-133 run, because inserting it in sequence would misrepresent when it was recorded. A 2026-07-26 audit of ledger-number continuity found P-132 was the only number in the P-007…P-156 range that had **shipped to `master` with no PSD entry** — it survived solely as a passing clause inside P-135's prose ("5 intervening commits — P-131 docblock, P-094 VERIFIED flip, **P-132 gitignore**, P-133 test wave ×2"). AGENTS.md makes the PSD loop mandatory, so a merged repo change with no entry is exactly the drift the ledger exists to prevent. (The other absent numbers in range — P-010, P-015, P-016, P-029 — were checked and are abandoned claims that never reached `master`; no gap to fill.)
- **PROBLEM (evidenced, from the commit body):** two unrelated pieces of root-level clutter. (1) `.claude/settings.local.json` is machine-local (permissions only, no secrets) but was **neither tracked nor ignored**, so it surfaced as `??` on every `git status` — persistent noise that also risked being swept into a blanket `git add -A`. (2) `2026-07-15-ADOPTION-RUNBOOK.md` sat at repo root describing the p101/p102/p105 bundles, all merged to `master` for over a week; its own successor, `docs/2026-07-16-MERGE-RUNBOOK.md`, had carried the line "supersedes this file — delete it" since the day it was written. The stated 2026-07-18 blocker (a `Vault/Daily` cross-reference) had already dissolved when P-126 untracked `Vault/Daily`.
- **SOLUTIONS considered:** 1. *Track `settings.local.json`* — rejected: it is machine-local by definition, so tracking it would push one machine's permission set onto every clone. 2. *Leave the runbook as harmless history* — rejected: a root-level document that contradicts its own successor is a live trap for the next agent doing a cold read, and the successor already ordered its deletion. 3. *Ignore the settings file, delete the superseded runbook.* Chosen.
- **DECISION + evidence:** `.claude/settings.local.json` added to the existing `.claude/` ignore block (+1 line in `.gitignore`); `2026-07-15-ADOPTION-RUNBOOK.md` deleted (−171 lines). Verified before deletion that no other tracked file depended on it existing at root. Total diff: 2 files, +1 / −171. Off-perimeter, zero app-code contact.
- **Status:** SHIPPED (2026-07-23, `8ab9b1d`, authored by a Sonnet 5 session). Entry reconstructed 2026-07-26 to close the audit-trail gap; no code was re-touched.

### P-156 · Renderer leaf presentational components (`StatPill`, `Ring`, `SessionPill`, `Icon`, `PanelHead`, `RailSlot`, `Modal`, `Dropdown`) shipped with zero test coverage — **rescued**: the work existed only as untracked files on the operator's disk for two days, and its ledger claim collided with a merged number — SHIPPED (2026-07-26, branch `test/p156-leaf-component-jsdom`)
- **PROBLEM (evidenced):** Two problems, one entry. (1) *The coverage gap*: with the renderer-store vein at 24/24 and the P-129 chart-subtree jsdom program closed at P-133, a 2026-07-24 census of `src/renderer/components/` found 8 pure-props leaf components — the operator-facing legibility primitives (P3): `StatPill.tsx` (33 L), `Ring.tsx` (33 L, SVG geometry), `SessionPill.tsx` (33 L, glyph map), `Icon.tsx` (40 L), `PanelHead.tsx` (31 L), `RailSlot.tsx` (63 L), `Modal.tsx` (55 L), `Dropdown.tsx` (108 L, portal) — carrying **zero characterization coverage**. Two of them (`Modal`, `Dropdown`) attach `window` listeners in effects with no regression pin on their cleanup branch — the PR #6 / §2.5.7 leak class the repo guards everywhere else. (2) *The custody failure*: the 2026-07-24 work-layer session wrote those 8 suites and **deliberately left them unstaged "for operator review"**, with its ledger claim numbered **P-137**. That session never closed the loop. By 2026-07-26 the files had sat **untracked in `mc4` for two days — present in exactly one place on earth, one `git clean` from gone** — and `P-137` had meanwhile been taken by the merged RS-0.3 policy-files entry (the P-090 concurrent-agent collision predicted in that session's own coordination note, never resolved). The suites had also **never been executed**: that session recorded `require('jsdom')` alone exceeding its 45 s sandbox ceiling over 9p-mounted `node_modules`, so it shipped them on static review with "CI is the declared arbiter" — and then never opened a PR for CI to arbitrate.
- **SOLUTIONS considered:** 1. *Leave them untracked pending the original session's review* — rejected: that review was two days overdue and the custody risk was the dominant term; unmerged work on one disk is not "pending", it is unbacked. 2. *Commit as-is under the colliding number* — rejected: two P-137s in one ledger breaks the PSD queue's only index. 3. *Verify first, then rescue under the next free number.* Chosen: run the suites on a toolchain that can actually execute them, renumber to the current free head, and put them in git behind a PR so CI arbitrates as the original session intended.
- **DECISION + evidence:** work preserved byte-identical (backed up before any git operation, then restored onto a branch cut from current `origin/master`), renumbered **P-137 → P-156**, executed for the first time in the `mc4-rust` worktree whose `node_modules` is Node-ABI. **`src/renderer/components/` — 10 files / 51 tests, 0 failed** (8 rescued suites plus the 2 already resident). Subjects **byte-unchanged** — additive test files only, no production edit. Full gates re-run on the branch below. The suites do what they claimed: `Modal` and `Dropdown` assert listener/portal removal on unmount (Escape and mousedown inert post-unmount), pinning the repo's most recidivist defect class on its two live interactive primitives.
- **Process note (worth more than the tests):** this is the second time the P-090 concurrent-agent pattern has cost real work — a session claimed a ledger number another session had already merged, then left its output unstaged and un-PR'd. The repo's own architecture already had the answer and it was not followed: RS-0.1 designates `mc4-rust` as the worktree where in-flight branches live and `mc4` as the one that stays on `master`. Work parked as untracked files in `mc4` is outside that scheme entirely. **Standing rule going forward: a session that produces files it cannot gate still puts them on a branch and pushes — "unstaged for operator review" is not a resting state, a PR is.**
- **Status:** SHIPPED. `mc4` working tree returned to clean and fast-forwarded to `origin/master`.

### P-155 · **The AI decision path is not replay-deterministic** — unseeded `Math.random()` in the depth ladder reaches ~25 % of every confidence score, blocking Oracle L1 decision parity — **CLOSED by P-157** (was OPEN pending operator ruling; found by the RS-1.3 driver 2026-07-25, ruled and shipped 2026-07-26)
- **PROBLEM (evidenced, measured):** With the autonomous trader cycling under the RS-1.3 golden driver, two captures of the *same* tape on the *same* virtual clock produce different decisions. Identical symbol (`NVDA`), identical `tickIndex` (360), identical virtual `createdAt` (1784880030000) — **confidence `0.3520162749933342` vs `0.36683881944775815`** ("bullish · confidence 35 % < 60 %" vs "37 % < 60 %"). The chain, all three links verified at file:line: (1) `trading-engine.ts:1538` — `getAiDecision` does `const depth = this.depth.get(symbol)` and passes it to `brain.decide(symbol, quote, ind, depth)`; (2) `depth-feed.ts:87-91` — `jitterFor()` mutates the ladder with **four unseeded `Math.random()` calls on every tick** (`Math.floor(Math.random() * arr.length)` twice, `(Math.random() - 0.5) * 0.18` twice); (3) `brain.ts:29-30, 46-47, 86-105` — the top of that ladder becomes the `depth_imbalance` (weight **0.15**) and `microprice_dev` (weight **0.10**) features. So **a quarter of the feature weight behind every AI confidence is drawn from an unseeded RNG**, on the live path as well as under replay.
- **Why it matters beyond the oracle:** the Rust engine cannot reproduce a number drawn from `Math.random()`, so this is a hard blocker for Oracle **L1 decision parity** — the stratum Appendix A.3 says has *no tolerance* ("any mismatch is a defect"). Until it is seamed, the parity harness can measure the engine's *state* but not its *judgement*, which is most of what the rewrite exists to prove. Separately, and independent of the rewrite: the same unseeded jitter means the live terminal's stated confidence is not reproducible from its own inputs, so a post-hoc audit of "why did it size that trade" cannot be reconstructed exactly.
- **SOLUTIONS considered:** 1. *Exclude depth from Oracle L1/L2 and move on* — this was the RS-1.3 draft's position and **it was wrong**: excluding depth from the *captured stream* does not remove it from the *computation*. The confidence still varies; the exclusion would just hide the variance from the tripwire that found it. 2. *Seed `DepthFeedService` with the injected `SeededRng` (RS-0.7)*, the same seam `MarketSimulator` already uses via `env.rngSeed`. Smallest change, matches the existing pattern, makes both engines reproducible. **Recommended.** 3. *Make `getAiDecision` ignore depth under replay* — changes decision semantics between live and replay, which breaks the premise that a replay reproduces the session; rejected. 4. *Zero the two depth features under replay* — same objection, and it would silently change live-vs-replay behavior for a value the operator sees.
- **DECISION:** none — **operator ruling required.** `depth-feed.ts` is perimeter-adjacent (its output feeds sizing via confidence), so per §2.4 no code changed. Pinned rather than left as prose: `capture.determinism.test.ts` carries a deliberately-inverted test that asserts the divergence exists, asserts it is in `kind=autonomy.decision`/`confidence`, and **fails the day the RNG is seeded** — with a docblock telling the fixer to delete it, enable `autonomy` for the corpus capture, and regenerate the goldens. Consequence today: `autonomy` defaults **off** in the driver and the archived golden carries **zero** decision records.
- **Status:** ~~OPEN~~ → **SHIPPED via P-157** (2026-07-26). The operator ruled Option 2; the depth ladder is seeded, the inverted tripwire above fired and was flipped to its positive form, and the golden was regenerated with `autonomy` on (146 → 1,396 records, 0 → 14 decisions). Blocks cleared: RS-1.4 parity on the decision stream, plan risk R3.

### P-154 · RS-UP-1 / RS-1.3 slices 2–3 — the golden-capture driver runs: headless engine boot, virtual clock, hermetic sandbox, and the **double-run determinism proof passing on the real corpus** — SHIPPED (2026-07-25, branch `feat/rs-1.3-golden-driver`)
- **PROBLEM (evidenced):** P-153 shipped the golden *format* and measured the engine seam, but the driver itself did not exist. Until it did, M1 could not close and RS-1.4/RS-1.7 had nothing to consume. Worse, the plan's biggest open risk was still unmeasured: **R3 — "TS replay is nondeterministic, oracle strategy collapses"**. RS-0.6 had found the replay path is *not* decision-deterministic as written (timer-driven decisions; `ReplaySource.currentReplayTime()` is literally `anchorReplayTs + (Date.now() - anchorWallTs) * speed`), and RS-0.7 designed a remedy on paper. Nobody had run it. Finding out at RS-10.1 costs a quarter (plan 5.1).
- **SOLUTIONS considered:** 1. *Standalone `tsx` script* — the plan's first suggestion, but `import { app } from 'electron'` under bare Node resolves to the npm package's path string, so `app` is `undefined`, `openDB()` throws into its `catch`, and persistence silently degrades to `NullDB` — a driver that emits empty goldens (P-097 class). Intercepting module resolution would need loader hooks. 2. *Run the capture under vitest.* Chosen. `vi.mock('electron')` is real module interception, `vi.useFakeTimers()` **is** `@sinonjs/fake-timers` (so RS-0.7's remedy arrives with **zero new dependencies**), `@shared` aliases already resolve, and `vitest.config.ts` already includes `scripts/**/*.test.ts` — so the double-run proof lands in CI **without a workflow edit**. 3. *Separate capture and proof paths (script writes archives, test proves determinism)* — rejected: two implementations drift, and the archived golden would then be bytes CI never verified. The writer now archives the *proven* run, so archive and proof cannot disagree.
- **DECISION + evidence:** seven modules under `apps/satex-terminal/scripts/oracle/`, **68 new tests**, zero engine edits. `sandbox.ts` (scratch tree + pinned env + closed network), `electron-stub.ts`, `corpus.ts` (reader + deterministic synthesizer), `importer.ts`, `capture.ts` (the driver), `archive.ts`, plus `capture.determinism.test.ts` — the proof. Design record and regeneration procedure: `docs/plans/rs-1.3-golden-driver.md`.
  - **R3 IS MEASURED — GREEN FOR STATE, RED FOR DECISIONS (P-155).** Two independent captures over the rescued P-143 tape (35,658 rows, 18 symbols, 9.15 min) produce **byte-identical goldens**: sha256 `348ec08f312bba2763d2f2806bb6ee681e0eb76f76c9747b5f4a270798cb66d7`, 146 records (30 L1 / 116 L2), 92 KB, 35,622 ticks emitted, ended `end-of-tape`. Corpus sha verified against the index first (`1a202d2f…`). The proof is **not vacuous**: a companion test asserts the two runs emit genuinely *different raw ids* (`shortId()` mixes `Date.now()`+`Math.random()` and keeps a process-wide counter), so equality holds only because the golden normalises ids by first-occurrence order — the normalize-in-golden ruling is load-bearing, exactly as intended. A third test proves the hash *changes* when the tape changes, so equality is not an artifact of a constant stream.
  - **The harness cannot arm live mode, by construction.** `dialog.showMessageBox` returns `response: 0`; `index.ts` arms only on `response === 1` (C6 / P-148). It refuses through the same branch a human pressing Cancel takes, not a special harness code. `safeStorage.isEncryptionAvailable()` returns `false`, so no stored credential resolves even if the operator has keys saved. Every capture records the attestation: **0 dialogs answered**.
  - **A real hermeticity defect was found and closed by this work.** The first spike logged `edgar poll failed — TypeError: fetch failed`: `EdgarService.start()` arms a 10 s timer, and since virtual time advances by the whole replay duration, that timer fires on any tape longer than ten virtual seconds and reaches for `https://www.sec.gov`. It failed only because that machine had no route at that instant — **on a connected machine it would have injected live SEC filings into the decision stream.** `fetch` is now refused and recorded; the archived manifest names the blocked target.
  - **P-097 law applied to the importer.** `NullDB` accepts writes, returns `[]`, throws nothing, and `insertTickBatch` returns `rows.length` regardless — an import under it would report 35,658 rows written over an empty table. Three checks now stand between an import and a capture: the file carries a real `SQLite format 3` header; bounds **read back out of the store** match what went in; and the manifest hash recomputed from imported bounds must equal the one the recorder sealed (the P-143 lesson — prunes had left manifests claiming 13.06 M ticks against 35,658 survivors). A NullDB-shaped store is unit-tested to be rejected. `archive.ts` refuses an empty golden or one from a stalled run for the same reason: a short golden is indistinguishable from a good one once on disk.
  - **THE TRIPWIRE FIRED — see P-155.** Turning the autonomous trader on (opt-in, added while CI ran on this PR) made the double-run proof **fail**, and the failure localised to one field: `autonomy.decision.confidence`. Root cause traced to unseeded `Math.random()` in the depth ladder reaching ~25 % of the feature weight behind every confidence. **This corrects an earlier claim in this very entry's first draft**, which described depth as merely "not captured" and therefore harmless — excluding depth from the captured *stream* does not remove it from the *computation*. R3 is therefore **split**: reproducible for the captured state/gate surface, **not** reproducible for the decision stream. Found in week one of M1, which is precisely what plan §5.1 said this task was for ("finding out at RS-10.1 costs a quarter").
  - **CI coverage is honest about the corpus.** `Vault/Backtests/` is gitignored, so CI cannot see the recorded tape. The proof therefore also runs against a synthetic tape built from integer arithmetic alone (no `Math.random`, no `Date.now`) — byte-identical on every machine — and the corpus case is reported as **skipped, never passed**, when absent, with an always-running `corpus availability` test that states which mode the run was in.
- **Gates (measured, operator hardware, Node-ABI native module):** typecheck **exit 0** · lint **exit 0** · knip **exit 0** (zero oracle findings) · vitest **167 files / 2,188 tests, 0 failed** — baseline moves from P-153's 160 / 2,120. Full oracle lane runs in ~2.3 s including two 35k-tick replays.
- **OPEN — operator decisions (three):** (1) **P-155 — seed the depth RNG.** Highest value of the three: until it lands, the parity harness measures the engine's state but not its judgement. (2) **Should goldens be committed?** `Vault/Backtests/*` is gitignored, so the archived golden lives only on operator hardware — the same exposure that cost 49 of 50 tapes in P-143. Unlike the corpus (D-017: 4.6–12 GB for 20 sessions, which disqualified git), a golden is **92 KB**; twenty is ~2 MB. Recommend a `!Vault/Backtests/goldens/` negation so a parity claim can cite a committed artifact. (3) RS-1.2 corpus breadth (≥20 sessions, five regime tags) still needs operator recordings — that, not this task, is now the M1 critical path.
- **Status:** SHIPPED. RS-1.3 is complete across all three slices. Next: RS-1.4 parity harness (consumes `golden.ts` + the archived manifest), RS-1.7 oracle mutation test (perturb a golden field, demand the differ exits non-zero naming it).

### P-153 · RS-UP-1 / RS-1.3 — golden-capture driver **CLAIMED**; slice 1 shipped: the golden record contract (canonical serialisation · id normaliser · stream hash) with 30 tests — SHIPPED (2026-07-25, branch `feat/rs-1.3-golden-contract`; slices 2–3 closed in P-154)
- **PROBLEM (evidenced):** RS-1.3 is the plan's named sleeper ("budget for it", 7.2) and everything downstream of M1 waits on it. Two things had to be settled before a line of capture code was worth writing: (1) the golden **format**, because the driver, the drift differ (RS-1.4) and the mutation test (RS-1.7) must all agree on it byte-for-byte, and (2) the driver's **runtime shape**, which was `UNKNOWN — VERIFICATION REQUIRED` (plan 0.F). Both are now measured, and the format is code.
- **SOLUTIONS considered:** 1. *Build the whole driver in one pass* — it is an L-size task whose parts have different risk profiles, and plan 5.0's GO/NO-GO forbids merging a partial port ("partial ports do not merge; they park on their branch"), so a one-pass attempt lands either nothing or something ungated. 2. *Slice it so every slice is independently complete and gated*: (1) format contract — pure, dependency-free, testable anywhere; (2) determinism harness — electron stub + fake timers + corpus→scratch-DB materialisation; (3) engine boot + decision subscription + the double-run proof. Chosen. 3. *Write the format inline inside the driver and extract later* — it would ship untested and un-reusable, and RS-1.4/RS-1.7 would fork their own copy.
- **DECISION + evidence (slice 1):** `scripts/oracle/golden.ts` + `golden.test.ts`, zero runtime deps (`node:crypto` only). Implements the **operator's normalize-in-golden ruling** (P-143): `IdNormalizer` maps generated ids to first-occurrence-ordered placeholders per prefix, over an **allowlist of the nine prefixes `shortId()` is actually called with** (measured by grep over `src/main`), matched on the structural id shape so a symbol, an enum, or `order_flow` can never be mangled; longest-prefix-first so `order_` is not read as `ord`; idempotent; ids normalised in **object keys** as well as values (the engine keys its in-flight index by order id). `canonicalize()` sorts object keys (property-insertion order is an engine detail that must never fail a parity run), preserves array order, emits numbers via JS shortest-round-trip (Appendix B.3 — same text Rust `{}` gives a finite `f64`), and **throws** on NaN/Infinity/undefined instead of letting `JSON.stringify` hide them as `null` or a dropped key. `hashStream()`/`GoldenStream` produce the exact bytes a golden file holds, LF-terminated — the value RS-1.3's double-run proof compares. Tests pin the properties that would let a *false* parity pass: 500 distinct ids never collide onto one placeholder, key-order changes do not move the hash but value changes do, and a **one-ULP** float change is a divergence (prefiguring RS-1.7).
- **Runtime facts measured for slices 2–3 (plan 0.F answered):** the engine's seam needs **zero engine edits** — `new TradingEngine()` (`index.ts:148`, no-arg) → `await engine.initialize()` (`:1132`) → `engine.startReplay(req)` (`trading-engine.ts:1725`), with the decision stream available through the public `onOrders/onAccount/onAutonomousDecision/onRiskGatesUpdate/onTradeClosed/onFeedStatus/onReplayStatus` listeners (`:899-1044`). The electron surface that must be stubbed is small and enumerated: `app.getPath('userData'|'downloads')`, `safeStorage.{isEncryptionAvailable,encryptString,decryptString}`, `dialog.showMessageBox`, `powerMonitor.{on,off}`, `BrowserWindow.focus`, `shell.openExternal` — and the stub's `dialog` **refuses**, so the harness cannot arm live mode by construction (a safety-positive property, not a limitation). `better-sqlite3` ships Electron-ABI in this worktree, so the driver runs under Node only after `npm rebuild better-sqlite3` — CI already does exactly this before vitest (`ci.yml:53-54`), so the double-run proof can run in CI as the plan requires; the rebuild was performed in `mc4-rust` this session (independent `node_modules` — the operator's `mc4` dev environment is untouched). Determinism dividend found while designing: with the virtual clock pinned to the tape's first tick, the engine's own 48 h prune cutoff computes to *before* the corpus's first row, so background maintenance provably cannot delete the corpus mid-run (`trading-engine.ts:383-386`). A hard assertion that the real DB is open (never `NullDB`) is a slice-2 requirement — a silent no-op store would emit empty goldens, which is the P-097 false-green class.
- **Gates (measured, operator hardware, Node-ABI native module):** typecheck **exit 0** (both tsconfigs; `tsconfig.node.json` now includes `scripts/oracle`) · lint **exit 0** (`eslint src tests scripts/oracle` — the lint script was extended so the new code is actually covered) · test **exit 0, 2,120 passed / 160 files, 0 failed** (new suite alone: **30 passed**) · knip **exit 0** (`knip.json` entry+project extended to `scripts/oracle`, so the dead-code gate can see the oracle lane and a future `@sinonjs/fake-timers` devDependency will register as used). Honest note: before the native-module rebuild, `persistence.test.ts` failed 38/38 on this hardware via the documented NullDB fallback — an Electron-vs-Node ABI condition, not a code defect; the rebuild took the full suite to 160/160. Baseline moved: the P-100 record of 1,668 tests / 126 files now measures **2,120 / 160** (`ARCHITECTURE.md` §4 refresh is a separate follow-up, not folded in here).
- **Status:** IN-PROGRESS. Next: slice 2 (electron stub + fake-timer harness + corpus→scratch-DB importer), then slice 3 (headless boot, decision subscription, double-run hash proof in CI). Corpus for the proof is the P-143 rescue tape (9.15 min, 18 symbols) — sufficient for every mechanical property M1 requires.

### P-152 · `AutonomousTrader.stop()` does not stop the in-flight cycle, and a stop→start during a cycle **leaks an armed timer** — paper orders can land after the operator disables autonomy, and the cycle rate silently doubles — OPEN (operator ruling; autonomous lifecycle)
- **PROBLEM (evidenced):** `autonomous-trader.ts:108-114` — `stop()` clears the pending timer and sets `enabled: false`, but carries no cancellation token, so a `runCycle()` already past its entry guards (`:135-137` check `this.cycling` and `this.status.enabled` **only at entry**) runs to completion through its `await`s and can still submit paper orders after the operator pressed stop. Second, sharper defect: `runCycle`'s `finally` (`:158-160`) unconditionally runs `this.cycling = false; this.scheduleNext()`. Sequence — cycle in flight → `stop()` (timer cleared, `enabled:false`) → `start()` (`enabled:true`, schedules timer **A** into `this.timer`) → the in-flight cycle's `finally` fires `scheduleNext()`, which passes its `enabled` guard and **overwrites `this.timer` with timer B, orphaning A**. A is still armed and now unreachable: no handle exists to cancel it, so the decision loop runs at **double** the configured rate for the rest of the session, and each subsequent stop/start during a cycle doubles it again. This is the §2.5.7 leak class (PR #6 · P-041 · P-043 · P-046 · P-091 lineage) on the decision loop itself.
- **SOLUTIONS considered:** 1. *Cancellation token + single-owner timer handle*: `stop()` sets an epoch/`AbortController` the cycle checks after every `await` and before submission, and `scheduleNext()` refuses when the cycle's epoch is stale — closes both halves. 2. *Guard `scheduleNext()` on "no timer already armed"* — fixes the doubling only; orders still land after stop. 3. *Await the in-flight cycle inside `stop()`* — makes stop honest but turns a UI action into an unbounded wait on network/broker I/O.
- **DECISION:** **Deferred to operator ruling — no autonomous change** (the autonomous trader is inside the §2.4 perimeter's blast radius even though it is paper-only). Recommendation: **(1)**, one human-gated PR, with a regression test that pins "stop during cycle ⇒ zero submissions after stop" and "stop→start during cycle ⇒ exactly one armed timer". Rust-side consequence, binding on RS-5.8: the ported `AutonomousTrader` takes a `CancellationToken` and a **single-owner** timer handle so the doubling is unrepresentable rather than merely untested (tokio `JoinHandle` ownership + `select!` on the token), and "stop means stopped" gets a truth-table test. Scar-ledger row added under the leak class.
- **Provenance:** Track X systems lens; mechanism re-verified line-by-line by the session before filing.
- **Status:** OPEN (operator ruling). Zero code changed.

### P-151 · `TradingEngine.shutdown()` reads the database **after** `closeDB()`, lazily reopening it and recreating the WAL that the checkpoint just truncated — OPEN (shutdown ordering; low severity, high tidiness value)
- **PROBLEM (evidenced):** `trading-engine.ts:870` calls `db.closeDB()` (which checkpoints `wal_checkpoint(TRUNCATE)` and drops the handle), and the end-of-session LEARNINGS block that follows calls `db.listBrainParams()` at `:885` for `weightsAtEnd`. `persistence.ts:64-66`'s `openDB()` is lazy — `_db` is null after close, so the read **transparently reopens** the database, re-runs the idempotent schema/PRAGMA init, and recreates a WAL file immediately after the code that carefully truncated it. The whole block is wrapped in `try/catch → log.warn`, so nothing surfaces; the process can then hard-exit (P-072's 5 s watchdog, `app.exit(0)`) with that fresh handle open. No corruption has been observed and SQLite is robust to this, but the shutdown sequence's stated intent — checkpoint, truncate, close cleanly — is not what runs.
- **SOLUTIONS considered:** 1. *Move the `weightsAtEnd` read (and any other DB read in the LEARNINGS block) above `db.closeDB()`* — one-line reorder, restores the intended ordering, zero behavior change otherwise. 2. *Snapshot everything the shutdown path needs into locals at the top of `shutdown()`* — more robust against the next reader added below the close, slightly larger diff. 3. *Make `closeDB()` poison the module so a post-close read throws instead of silently reopening* — surfaces the whole class, but converts a benign warning into a shutdown-path exception and needs an audit of every call site first.
- **DECISION:** **Deferred to operator ruling — no autonomous change** (shutdown ordering on the shipping terminal). Recommendation: **(2) now, (3) in Rust**: the Rust port expresses shutdown as a *consuming* operation (`fn shutdown(self)`), which makes "read after close" a **compile error** instead of a logged warning — recorded against RS-4.2's DB-actor contract. **Not re-verified by this session and therefore not asserted:** the systems lens also reports that `shutdown()` has no re-entrancy guard while `index.ts:1167`'s comment claims `isQuitting` prevents double invocation from `window-all-closed` + `before-quit`. Treat that as a lead to verify, not a finding — see `scratchpad/trackx-lens-c-systems.md`.
- **Provenance:** Track X systems lens; the read-after-close ordering re-verified at `file:line` by the session.
- **Status:** OPEN (operator ruling). Zero code changed.

### P-150 · `reconnectAlpaca()` has no recovery path — a failed reconnect leaves a **dead `LiveMarket` installed as the engine's data source** and returns `{ok:false}`, which is the silent-stale-feed shape §3.2 forbids — OPEN (perimeter-adjacent; operator ruling required)
- **PROBLEM (evidenced):** `trading-engine.ts` replaces the market and rewires it *before* connecting — new `LiveMarket`, `this.session.orders.onUpdate(...)`, `installMarketWiring(this.market)` (`:1439-1441`) — then `await this.session.connect()` (`:1442`). The `catch` at `:1449-1452` does exactly two things: `log.error('reconnect failed', …)` and `return { ok: false, reason }`. There is **no rollback**: the previous working source is gone, and the engine is left holding a `LiveMarket` whose session never connected. Contrast its sibling 50 lines earlier: `setDataSource`'s failure path (`:1386-1397`) reconciles by restarting the simulator locally. Constitution §3.2 is explicit — *"Stale data is poison. Degrade loudly, never silently: if a feed dies, the correct behavior is HALT-and-surface, not fallback-to-stale"* — and §3.8 requires prolonged outage ⇒ HALT trading. A dead installed feed satisfies neither: it neither serves data nor halts, and the only signal is one `log.error` plus a boolean the renderer currently discards (the reconnect IPC is fire-and-forget from `TopBar`).
- **SOLUTIONS considered:** 1. *Reconnect into a staging source and commit it only on successful `connect()`; on failure restore the prior source untouched* — the correct shape (prepare/commit), no user-visible state change on a failed attempt. 2. *On failure, HALT and surface: tear the dead source down, mark the feed `FAILED`/degraded, and push that to the renderer* — most faithful to §3.2/§3.8, and it needs the honest degraded state the session FSM does not yet have. 3. *Fall back to the simulator like `setDataSource` does* — **rejected**: substituting synthetic data for a failed live feed is exactly the silent-fallback §3.2 forbids, and it would put SIM data behind a live-labelled session (the SIM badge would show, but the operator asked for live and got sim without a decision).
- **DECISION:** **Deferred to operator ruling — no autonomous change** (§0.3: the data plane feeding the order path). Recommendation: **(1)+(2) together** — prepare/commit so a failed reconnect changes nothing, plus an explicit surfaced `FAILED` state when there is no working source left; never (3). **Rust-side consequence, binding on RS-7.6 and Track X:** reconnect is modelled as a state machine with a *staged* candidate source and a first-class `Degraded`/`Halted` state; the transition "commit new source" is only reachable from "connect succeeded", which makes the dead-source state unrepresentable. Note for RS-L8/L1: because this changes what the engine *does* on failure, it is an operator-ruled improvement, not a port — it ships as its own reviewed change on whichever side the operator chooses, never folded into a port commit.
- **Provenance:** Track X systems lens; re-verified at `file:line` by the session (bare catch confirmed, sibling fallback confirmed at `:1386-1397`).
- **Status:** OPEN (operator ruling) — the highest-severity *data-integrity* finding of the 2026-07-25 session.

### P-149 · Display honesty — the status ribbon ships four hardcoded metrics (incl. a fabricated `SHARPE 2.10`) in the same visual grammar as real computed values, and the live-arming checklist renders an interlock that is both inverted and decorative — OPEN (Directive 0.1 / 0.4 violation on the operator's screen; renderer-only, operator ruling required)
- **PROBLEM (evidenced):** (1) `renderer/components/BottomBar.tsx:47` `LIQ DEPTH "top-of-book · ok"`, `:49` `CVD "buy-init bias"`, `:50` `SLIPPAGE "1.4 bp · good"`, `:51` `SHARPE "2.10 · rolling"`, `:71` `LOG "● tape · ok"` are **string literals**, rendered through the same `<Item k= v=>` component as the genuinely computed `P&L · TODAY`, `EQUITY`, `BP`, `VaR 95`, `EXPOSURE`, `DXY`, `TNX`. Nothing on screen distinguishes measured from invented. A hardcoded Sharpe ratio sits a few panels from the DISCIPLINE panel (P-100) that exists specifically to report *deflation-adjusted* edge honestly (P-096) — the fabricated number undercuts the flagship feature's whole premise, and Directive 0.1 forbids invented metrics without qualification. (2) `renderer/components/modals/LiveModeModal.tsx:60` — `const endpointOk = status?.paperOnly === false ? false : true` renders ✗ **exactly when the endpoint is correctly live**, i.e. the checklist marks the required condition as failed; and `:62` `allOk = killOk && dailyLossOk && capOk` **excludes** `endpointOk`, so the mark gates nothing (`:102` uses `allOk`). An arming checklist that shows a red ✗ in the normal success case, and whose red ✗ blocks nothing, trains the operator to ignore red marks on the one screen where red must mean stop.
- **SOLUTIONS considered:** 1. *Delete the five fabricated items; fix the `endpointOk` polarity and include it in `allOk`* — smallest honest diff; the ribbon loses cosmetic density it never earned. 2. *Wire each metric to real data* (`tca.ts` already computes slippage; CVD needs the footprint store; rolling Sharpe exists in `self-eval.ts` with PSR/DSR) — best end state, materially larger, and Sharpe-without-deflation would contradict P-096 unless it ships with its significance verdict. 3. *Label them `demo`/`—` until wired* — honest and cheap, but leaves dead furniture on a P3 surface.
- **DECISION:** **Deferred to operator ruling — no autonomous change.** Both files are renderer surfaces of the shipping terminal, outside this session's Rust mandate, and item (1) is partly a product-taste call (which metrics deserve ribbon space) which §2.3's judgment boundary reserves to the operator. Recommendation on record: **(1) for the fabricated five now, (2) for `SLIPPAGE` and `SHARPE` later** (both have real sources, Sharpe strictly beside its PSR/DSR verdict), and **(1) unconditionally for the `endpointOk` polarity + `allOk` inclusion** — that half is a single-answer defect on an arming interlock display, not a taste call. Rust-side consequence: Track X's redesigned state surfaces inherit a hard rule — *every rendered value is either measured or explicitly marked unknown*; `UNKNOWN` is a first-class output (§3.3), a placeholder string is not.
- **Provenance:** found by the Track X ergonomics lens (design fleet, 2026-07-25), independently re-verified at `file:line` by the session before filing (Directive 0.5 — a subagent report is not authority).
- **Status:** OPEN (operator ruling). Zero code changed.

### P-148 · Doc-truth — the live-arming ceremony is a **native OS dialog click**, not a typed phrase; the typed-phrase check was deliberately removed as a *weaker* design on 2026-05-16, but CONSTITUTION §1.4/§2.4/§3.7 + Appendix A and plan RS-UP-1 RS-8.4 all still specify "typed-phrase native dialog" — DECIDED (documents corrected; RS-8.4 spec amended before it is claimed)
- **PROBLEM (evidenced):** `main/index.ts:817-861` implements `LIVE_MODE_SET` and its own docblock records the reasoning: *"Adversarial finding C6 (2026-05-16) — XSS-resistant live-mode interlock … This replaces the prior `confirmPhrase` string-equality check, which any in-process code could satisfy by hardcoding the known string."* Authorization is a click on an OS-rendered `dialog.showMessageBox` (buttons `['Cancel', 'I accept real capital']`, `response !== 1` ⇒ refusal at `:855-858`). No typed phrase exists anywhere in the arming path. Meanwhile CONSTITUTION v3.1 states "a typed-phrase native dialog the operator completes personally" (§1.4), "typed-phrase native dialog" (§2.4, §3.7 table) and defines the glossary term "Arming ceremony" as typed-phrase (Appendix A); RS-UP-1 §5.5 RS-8.4 instructs the Rust port to build "typed-phrase dialog … proven by a test asserting the arming state transition requires the dialog's nonce".
- **SOLUTIONS considered:** 1. *Treat the code as truth, correct the documents, and amend RS-8.4 to port the C6 design (native dialog + nonce) while keeping the task's real contribution — the tests the TS side never had (P-094)* — the honesty axiom's prescribed move (§0.5: "the code is the truth and this file has a bug"). 2. *Reintroduce a typed phrase in Rust to satisfy the constitution* — **rejected as a safety regression**: C6 demonstrated that any in-process code can satisfy a known-string equality check, which is precisely the class Tauri's IPC surface would reopen. 3. *Add a typed phrase **inside** the native dialog (best of both)* — deferred, not rejected: OS message boxes have no text input, so this needs a custom always-on-top window whose input cannot be reached from the renderer; a real option for Track X, but it must be designed, not assumed, and it is strictly additive to the click gate.
- **DECISION:** Option 1. **RS-8.4's specification is amended before the task is claimed**: port the *measured* interlock — main-process-owned native dialog, no programmatic completion path, refusal on anything but the explicit affirmative — and add the missing test coverage; a typed phrase may only return via option 3 with an operator ruling. Constitution §1.4/§2.4/§3.7/Appendix A carry a doc-truth defect and are queued for the next review (P-095/P-104 lineage: same class, same remedy). Stale UI copy in the renderer that still instructs the operator to type a phrase is folded into P-149's ruling. This is the **third** consecutive audit in which a document about this repo was wrong about this repo (P-095 ruleset, P-104 constitution drift, this) — Directive 0.5 keeps earning its place.
- **Provenance:** raised by the Track X interlock + ergonomics lenses, re-verified at `file:line` by the session, and the fleet's framing was **inverted** on verification: the lenses read a missing ceremony as a weakened terminal; the code shows a *deliberately strengthened* one and stale docs.
- **Status:** DECIDED. Documents to fix at next constitution review; RS-8.4 amendment recorded here and binding on whoever claims it.

### P-147 · The atomic-write contract protects exactly **one** of six safety-relevant state files; `tactics.json` is the highest-frequency writer and its torn-write failure mode releases restrictions (fail-OPEN) — OPEN (perimeter-adjacent; operator ruling required)
- **PROBLEM (evidenced):** `kill-switch-store.ts:62` defines `writeJsonAtomic` (tmp + rename) and `:87` is its **only** call site in `src/main` (verified by grep across non-test sources). The other five state stores write in place with a single `writeFileSync`: `live-mode.ts:31` (**the live-capital arming state itself**), `tactics.ts:80`, `alpaca-mode.ts:35`, `self-eval-store.ts:31`; `funded-account-store.ts:63` hand-rolls tmp+`renameSync` with a fixed `.tmp` name and no `fsync`. Failure direction matters: `tactics.ts:77`'s corrupt-load fallback is `{ version, history: [], graduated: false }`, and `graduate()` logs *"TACTICS GRADUATED — pre-trade gate now active"* (`:174`) — so a torn `tactics.json` yields **both** `graduated:false` (the pre-trade gate reverts to pass-through) **and** `history:[]` (the MAX_DRAWDOWN veto's entire evidence base is erased, clearing the P-131 latch). It is also the most frequently written of the six — saved on every closed trade — which makes it the likeliest to be interrupted. `live-mode.ts` compounds it: arming state, no atomic write, no tests (P-094).
- **SOLUTIONS considered:** 1. *Promote `writeJsonAtomic` to a shared util and adopt it in all six, unchanged in behavior* — smallest correct fix, but it touches the arming and graduation stores, so it is human-gated perimeter work by §0.3 (the P-094 ruling: even *adding tests* to interlock code needs the human). 2. *Fix `tactics.json` + `live-mode.json` only* — addresses the fail-open and the arming state, leaves a knowingly inconsistent contract behind. 3. *Fix nothing on the TS side; express the contract once in Rust (`satex-risk`, RS-8.3/8.5/8.6) where every state file goes through one atomic writer with crash-injection tests* — zero risk to the shipping terminal, but leaves the live terminal's fail-open in place for the rewrite's whole duration (months).
- **DECISION:** **Deferred to operator ruling — no autonomous change** (§0.3: this is the perimeter). Recommendation: **(2) now, (3) always** — `tactics.json`'s fail-open direction is the one that can quietly widen what the system is allowed to do, and that is worth a small human-gated PR today; `live-mode.json` rides along because arming state deserves at least the durability the kill switch has. Rust-side consequence, binding regardless of the TS ruling: in the Rust world **no state file may be written except through the single atomic writer**, enforced by module visibility, and each store's *corrupt-load direction* must be an explicit, tested decision — a store whose failure mode removes a restriction is a defect even if the write is atomic. Recorded in the scar ledger against RS-8.3/8.5/8.6.
- **Provenance:** Track X interlock lens; re-verified at `file:line` by the session.
- **Status:** OPEN (operator ruling). Zero code changed.

### P-146 · The kill chord has **no out-of-renderer path** — a wedged or crashed WebView removes the panic button at exactly the moment it is needed — OPEN (perimeter; operator ruling required)
- **PROBLEM (evidenced):** grep across `src/main` for `globalShortcut`, `Tray`, `accelerator`, and `Menu.setApplicationMenu` returns **zero** non-test hits. The only kill path is the renderer chord in `App.tsx` (arm-hold at `:264-279`, fire at `:273`). P-044 (error-boundary reachability) and P-098 (boot-intro fall-through) both hardened the chord *inside* the renderer — genuinely valuable, and they are exactly why this gap is easy to miss: the chord survives every failure the renderer can *contain*, and none of the ones that take the renderer itself out (WebView crash, GPU-process hang, unresponsive main-thread script, a wedged frame under load). Constitution §3.4 requires "reachable chord in every UI state"; "no UI at all" is a state the current implementation cannot serve. The engine-side kill switch itself is sound (`kill-switch-store.ts`, atomic writes, human-reset-only) — the defect is purely *reachability*.
- **SOLUTIONS considered:** 1. *Register an Electron `globalShortcut` in the main process as a second, renderer-independent path to the same `OrderManager` kill entry point* — closes the gap in the shipping terminal now; costs an OS-global hotkey (must be chosen so it cannot fire accidentally) and is main-process perimeter work. 2. *Add a Tray item + application-menu accelerator* — discoverable and harder to trigger by accident; still renderer-independent; larger surface. 3. *Defer entirely to Track X / RS-9.6 in Tauri* — the redesign already owns "kill chord global reachability incl. error states"; costs nothing now but leaves the shipping terminal one WebView crash away from having no panic button for the rewrite's duration.
- **DECISION:** **Deferred to operator ruling — no autonomous change** (§0.3). Recommendation: **(1) now** (smallest renderer-independent path, one main-process registration wired to the existing kill entry point, human-gated PR) **and (1)+(2) in Rust** as an RS-9.6 exit criterion, so the Tauri build ships with an OS-level chord *plus* a tray path and a test that proves the chord fires with the web view destroyed. Track X red line, asserted: **no redesign may make the kill path depend on the renderer being alive.**
- **Provenance:** Track X interlock lens; independently re-verified by the session (grep, zero hits).
- **Status:** OPEN (operator ruling). Zero code changed.

### P-145 · MAY-TACTICS graduation — the constitution's second arming-tier interlock — has **no main-process human gate**: it is a bare IPC call whose "requires explicit user confirmation" is a code comment, and the promotion it grants is persisted and one-way — OPEN (perimeter; operator ruling required)
- **PROBLEM (evidenced):** `main/index.ts:880` registers `register(IPC.TACTICS_GRADUATE, () => engine.graduateTactics())` — no dialog, no nonce, no confirmation of any kind. `tactics.ts:161-164`'s docblock states *"REQUIRES explicit user confirmation via UI — never auto-promoted. Per locked invariant: 'MAY-TACTICS leaving calibrating mode' is a user checkpoint"*, and the implementation (`:165-177`) enforces only three **performance** floors (`MIN_TRADES_FOR_ARMED`, positive expectancy, `MIN_WIN_RATE`) before `this.store.graduated = true; save(this.store)`. The human checkpoint is asserted in prose and enforced nowhere in the main process: any renderer-reachable path — an XSS-injected script in news content, brain/LLM output rendered into the DOM, a devtools paste, a mis-click, a bug — completes it, which is **precisely the C6 attack that live-mode arming was redesigned to defeat on 2026-05-16** (P-148). The two interlocks the constitution places in the same tier (§1.4, §2.4, §3.7 "Tactic graduation to broader autonomy → MAY-TACTICS interlock — human gate", §4.4 Rung 3) now have opposite protection: one is an OS-level dialog, the other is an unauthenticated command. Worse, it is **one-way**: the only `graduated: false` in `src/main` is the corrupt-load fallback (`tactics.ts:77`); no un-graduate command, IPC channel, or method exists, so a single spurious call permanently spends a decision the operator is supposed to make once, deliberately — and (P-147) a torn write is the only "reset", which also erases the drawdown veto's history.
- **SOLUTIONS considered:** 1. *Route `TACTICS_GRADUATE` through the same main-process native dialog the live-arming path uses (C6 pattern), quoting the measured metrics in the dialog body* — direct, small, reuses a design already proven against in-process attackers; main-process perimeter work, human-gated PR. 2. *Add an un-graduate command as well, so the decision is reversible* — reduces the blast radius of any future gap and matches "descending is automatic and requires no one's permission" (§4.4); needs a ruling on whether de-graduation should also require ceremony (recommend: no — protective actions stay frictionless, per the arming asymmetry the terminal already gets right). 3. *Leave TS as-is and fix it only in the Rust port (RS-8.5)* — zero risk today, but the exposure stays live for the rewrite's duration on the shipping terminal.
- **DECISION:** **Deferred to operator ruling — no autonomous change** (§0.3, §2.4: this is the perimeter, and RS-L3 puts every perimeter change behind a human review). Recommendation: **(1)+(2) now**, as one human-gated PR — the asymmetry between the two arming-tier interlocks is not a design choice anyone made, it is drift, and it is cheap to close. **Plan consequence, binding on RS-8.5:** RS-L1 says port behavior including bugs-in-amber *"unless separately ledgered and operator-approved"* — this entry is that ledger. RS-8.5 must **not** faithfully port a missing gate; it ports the gate the constitution specifies, with a `trybuild` compile-fail proof that no IPC-reachable type can mint the graduation grant, exactly as RS-8.4 does for arming. Scar-ledger row added.
- **Provenance:** Track X interlock lens; re-verified at `file:line` by the session before filing.
- **Status:** OPEN (operator ruling) — highest-severity finding of the 2026-07-25 session.

### P-144 · The 48-hour tick-retention prune deletes tape rows but leaves `tape_manifest` rows behind — 49 orphaned integrity records, and any session opened by id outside the replay picker throws — OPEN (TS-side; operator ruling required)
- **PROBLEM (evidenced):** `trading-engine.ts:383-386` schedules background DB maintenance 30 s after every `initialize()` with `pruneOlderThanMs: 48 h`; `persistence.ts:852-863` (`pruneOldTicks`) and `:917+` (chunked `pruneOldSessionTicks`) delete from `ticks` only — the docblock at `:843-846` deliberately preserves `sessions` rows, and nothing on the prune path touches `tape_manifest` (only the explicit replay-session delete does, `:726-727`). Measured on the live DB 2026-07-25: **50 sealed manifests claiming 13,064,935 ticks, 49 of them with zero surviving rows** (largest orphan: `ses_mppbze4kldg8001`, 3,896,974 claimed). Consequences, measured rather than assumed: (a) the operator's replay picker is **correct** — `listReplayableSessions` (`:637-663`) `INNER JOIN`s a live aggregate over `ticks`, so orphans never appear; (b) but `ReplaySource`'s constructor **throws** `session … has no tape rows` (`replay-source.ts:145-147`) for any path that opens a session by id without the picker — a stale `replay_bookmarks` row, a restored workspace, or tooling that reads `tape_manifest`; (c) 49 permanently unreconcilable integrity records accumulate, which is corrosive to the one table whose entire job is to make tape corruption detectable.
- **SOLUTIONS considered:** 1. *Delete the manifest row when a session's last tick row is pruned* (both prune paths) — restores the invariant "a manifest describes an existing tape"; loses the historical record that a tape once existed. 2. *Keep the row but mark it `pruned`* (schema addition) — preserves history and lets tooling distinguish "never sealed" from "sealed then pruned"; costs a migration. 3. *Leave the data as-is and make the read paths defensive* (picker already is; give `ReplaySource` a typed refusal instead of a bare throw, and have bookmark/workspace restore validate first) — smallest change, fixes the operator-visible half, leaves the dead rows.
- **DECISION:** **Deferred to operator ruling — no autonomous change** (off-perimeter but it is the shipping terminal's persistence layer, and (1) vs (2) is a data-retention taste call the operator owns). Recommendation: **(2)+(3)** — a `pruned` marker keeps the forensic trail the ledger philosophy prizes, and a typed refusal beats an exception on a path a stale bookmark can reach. **Corpus consequence (the reason this was found): a tape that lives only in the live database is not an artifact.** RS-1.2's rescue export (P-143) is what stands between the oracle and an empty corpus; that hygiene rule is now written into `docs/plans/rs-corpus-inventory.md` §3.
- **Status:** OPEN (operator ruling). Zero code changed.

### P-143 · RS-UP-1 / RS-1.2 [A] — corpus inventory: **the corpus does not exist** (49 of 50 recordings already pruned); the last surviving tape rescued to SHA-pinned artifacts hours before its expiry; D-017 quantified; golden-ID ruling recorded — SHIPPED (2026-07-25, `docs/plans/rs-corpus-inventory.md`, branch `docs/rs-1.2-corpus-inventory`)
- **PROBLEM (evidenced):** Phase 1's oracle needs ≥ 20 manifest-hashed recorded sessions spanning trend/chop/gap/halt/WS-drop (plan 5.2). Measured 2026-07-25, read-only against the live operator DB (`satex-app/satex.db`, 7,476,322,304 B): `ticks` holds **35,658 rows for exactly one session**, while `tape_manifest` holds 50 rows claiming **13,064,935** ticks — 49 orphaned by the 48 h retention prune (root cause filed separately as P-144). The single survivor `ses_mrynz0vlkf0x001` is a **9.15-minute overnight window** (`2026-07-24T08:14:26.709Z → 08:23:35.422Z`), 18 symbols (equities + ES/NQ/CL/GC futures + BTC/ETH), 3,897 ticks/min, max inter-tick gap 318 ms — and it was ~30 h old against a 48 h window that re-arms 30 s after every app boot. One more boot-day and Phase 1 would have started with zero replayable recordings.
- **SOLUTIONS considered:** 1. *Report the inventory as a document and let RS-1.3 read the live DB directly* — fastest, and **rejected twice over**: the tape expires within hours, and pointing the oracle at the operator's 7.5 GB working database makes every future golden depend on mutable, pruned, unversioned state. 2. *Export the surviving tape to SHA-pinned, prune-immune corpus files in a documented format, measure the real per-tick storage cost, and hand RS-1.3 a file contract instead of a database* — preserves the asset, satisfies plan 5.2's "read-only artifacts, a changed SHA is an incident", and answers D-017 with measurements. Chosen. 3. *Wait for the operator to record a proper corpus first* — leaves RS-1.3 blocked on market hours for no benefit; a 9-minute real tape proves every mechanical property M1 requires.
- **DECISION + evidence:** Option 2, executed. Corpus tape format v1 (LF JSONL: manifest line + fixed-key rows sorted `ts ASC, symbol ASC`, shortest-round-trip floats so `JSON.parse` in node returns identical IEEE-754 doubles per Appendix B). Exported and byte-verified (re-read, 0 NUL, 0 CR-CR, tail intact, re-parsed): `tape-ses_mrynz0vlkf0x001.jsonl` sha256 `1a202d2f52ed8c3f0bebecb2e99677a1a26e66f39df817297e71abe0c5280e55`, 5,507,967 B, 35,659 lines; `crypto-subsecond-candles.jsonl` sha256 `2341774f89d64846523937b29deb63ef97c7ae630a58c5c62547be1f6448032b`, 551,119 B (4,004 rows — the sub-second aggregator's only surviving evidence, also prune-exposed); `corpus-index.json` seeds the plan's `corpus.json`. Written to `Vault/Backtests/corpus/` (prune-immune, **gitignored** — so not yet a backup; P-014 lesson flagged for the operator) and the session scratchpad. **D-017 measured:** 154.5 B/tick raw, 42.6 B/tick gzipped (3.6×) ⇒ a dense session is 232–602 MB raw / 64–167 MB gzipped, and the 20-session target is **4.6–12 GB raw / 1.3–3.3 GB gzipped** — git is disqualified on GitHub's 100 MB/file limit alone; recommendation is artifacts outside git + a tracked kilobyte manifest carrying the SHAs, with bar-derived `hist_` days (3.6 MB each, deterministically re-importable per `historical-importer.ts:9-26`, but *synthetic microstructure* — 4 ticks/bar, so never valid evidence for the sub-second aggregator or depth feed) as the cheap breadth tier. **Operator ruling recorded (2026-07-25):** golden treatment of `id-generator` ids is **normalize-in-golden**, not seeded `Math.random` — ids are identity, not behavior, and leaving `Math.random` unseeded keeps the RS-1.3 double-run hash proof able to catch a *new* nondeterministic call (closes the ruling queued by P-139/P-140).
- **Gates:** read-only analysis of the live DB + two new documents in the RS worktree; no app code touched, TS four-gate bar unaffected (CI-arbitrated on the PR). Export tool byte-verifies every file it writes.
- **Status:** SHIPPED. **RS-1.2 [O] (operator recordings) is now on the M1 critical path** — six requested sessions with regime tags and a same-day export rule are specified in `rs-corpus-inventory.md` §6. RS-1.3 is **not** blocked: 9 minutes of real 18-symbol tick flow is sufficient for headless boot, fake-timer determinism, golden emission, ID normalization and the double-run proof. Synthetic adversarial set (scar classes) claimable as an RS-1.2 follow-on needing zero operator time.

### P-142 · RS-UP-1 / RS-1.6 — IPC contract inventory: the 122-vs-124 question RESOLVED to **124** — SHIPPED (2026-07-25, `docs/plans/rs-ipc-inventory.md`, branch `docs/rs-1.6-ipc-inventory`)
- **PROBLEM:** RS-9.2 (the Rust IPC surface) needs an exact, mechanical enumeration of every Electron IPC channel as its 1:1 completeness checklist; the plan flagged an unresolved 122-vs-124 discrepancy (grep heuristic vs literal-regex) that had to be settled by a real parse, not a count method (0.4).
- **SOLUTIONS considered:** 1. *Regex over ipc-channels.ts* — the very method that produced the ambiguous 122; rejected (Appendix C mandates AST/mechanical). 2. *Transpile the `export const IPC as const` via the app's own typescript to CJS, evaluate it in node, and walk the object at runtime* — exact, reproducible, immune to comment/formatting noise. Chosen. 3. *Hand-count* — not reproducible, not a gate.
- **DECISION + evidence:** **124** distinct channel constants, **zero duplicate string values** (confirms constitution §1.1's re-measured 124; supersedes the older 122). Direction split, verified against the preload surface + the single `ipcMain.handle` wrapper (`index.ts:675`): **92 invoke** (renderer→main, `.strict()` req/reply) + **26 event** (main→renderer push) + **6 flagged** command-shaped channels absent from `preload/index.ts` (CHART_DRAWINGS_GET/SET, CHART_PNG_EXPORT, FUNDED_ACCOUNT_SET_PROFILE/CLEAR/ADVANCE_PHASE) → RS-9.3 wiring-verification item. Schema source: `ipc-schemas.ts`, **124 zod schemas, all `.strict()`** — the DTO surface RS-9.2 satisfies 1:1 with `serde(deny_unknown_fields)`. `PUSH_CHANNELS` is an internal non-exported subset (derives the `PushChannel` type), not a separate channel set. Extractor is re-runnable (`scratchpad/ipc-inventory.mjs`); the inventory is RS-9.2's regenerable completeness skeleton.
- **Gates:** read-only mc4 analysis + one docs file in the RS worktree; TS four-gate bar unaffected (CI-arbitrated on the docs PR).
- **Status:** SHIPPED pending merge. Feeds RS-9.2/9.3 (shell lane, δ).

### P-141 · RS-UP-1 / RS-1.1 — satex-core kernel: UtcMillis, Provenance, Symbol, ids, Clock/SeededRng, error taxonomy — SHIPPED (2026-07-25, branch `feat/rs-1.1-satex-core`, PR #78)
- **PROBLEM:** Phase 1 needs the shared kernel every downstream crate depends on, and it must be a *faithful* port (RS-L1) with parity *measured*, not asserted (RS-L4) — especially the PRNG, whose bit-exactness is load-bearing for all future simulator/brain goldens.
- **SOLUTIONS considered:** 1. *Hand-write the kernel and eyeball it against TS* — fast, but "looks right" is not parity (0.4). 2. *Port + pin every ported value against reference vectors generated from the ACTUAL TS source under node* — slower, but every claim is reproducible from artifacts. Chosen. 3. *Pull a stats/PRNG crate for the RNG* — rejected: a library's PRNG/erf is a parity bug by construction (Appendix B); the TS mulberry32 IS the spec.
- **DECISION + evidence:** Ported `satex-core` (zero runtime deps, D-012; `#![forbid(unsafe_code)]` + `#![deny(missing_docs)]`). **mulberry32 + Box–Muller is BIT-EXACT** with `services/rng.ts`: `next_f64` asserted against raw IEEE-754 bit patterns generated by running the verbatim TS body under node (`scratchpad/gen-rng-vectors.mjs`, seeds 0/1/12345); `next_int` vectors pinned; gaussian to tight tolerance with the `f64::ln` last-ULP boundary **documented and flagged for the parity harness** (RS-2.x/3.1) — a ledgered ruling will resolve any future drift, never a silent tolerance widening. `ids` ports `id-generator.ts` (base36 encoder bit-exact w/ `Number.prototype.toString(36)`; clock+rng injected per RS-0.7 → reproducible under seed). `UtcMillis(i64)` (D-008, saturating, no chrono). `Source{Simulator,Live,Replay}` (§3.2) + `SideProvenance{Real,Inferred}` (`types.ts:38`); §3.2 validity judgment **deferred** to the data plane (documented, not invented — RS-L1). `Symbol` validated newtype + `is_known_crypto_ticker` (ports `trading-engine.ts:605` regex, flagged as fallback not canonical asset-class). `Clock` trait + `SystemClock` (the single sanctioned `SystemTime::now`) + `FixedClock`/`SteppedClock` (atomic `fetch_add`, race-free under `Sync` — a self-review catch). `clippy.toml` scoped unwrap/expect denies to non-test code (`allow-*-in-tests`), matching RS-0.3's documented "non-test code" wording (production code still fully denied). Self-adversarial pass done inline (fleet unavailable — session subagent limit); the operator may run a stronger-model quality review against the PR.
- **Gates (measured, operator hardware, vcvars64):** fmt 0 · clippy `-D warnings` 0 · test 0 (**31 satex-core tests**) · doc `-D warnings` 0 · machete 0 · deny 0. Windows arbiter + ubuntu-fast + TS Gates on PR #78 (windows now a required check).
- **Numbering note (honesty axiom):** the code commit message pre-stated "P-142"; the protocol-correct next-free committed number at ledger head P-140 is **P-141** — this entry is authoritative, the message had a bug. The sibling work-layer session's uncommitted "P-137" claim renumbers to the next free when it lands (P-090 committed-beats-uncommitted).
- **Status:** SHIPPED pending PR #78 merge. Next RS-1.x: RS-1.6 IPC inventory, RS-1.2 corpus (operator recordings), RS-1.3 golden driver.

### P-140 · RS-UP-1 / RS-0.7 — Clock/SeededRng injection design — SHIPPED (2026-07-24, `docs/plans/rs-clock-rng-injection-design.md`)
- **PROBLEM:** RS-0.6 (P-139) found ≈118 needs-injection clock sites and 9 timer-race sites; without a single enforced determinism strategy, the oracle (Phase 1) cannot make reproducible goldens and the Rust port would improvise per-site.
- **SOLUTIONS considered:** 1. *Per-site TS injection* — edits ~118 engine lines, violates plan 0.D zero-engine-edit rule, still doesn't fix timer races. 2. *Process-boundary fake time on TS (driver installs fake timers + faked Date; virtual time advanced tick-locked) + constructor-boundary `Clock`/`SeededRng` traits on Rust (pausable tokio time in parity mode)* — zero engine edits, collapses all (b)+(c) classes, one recorded timeline enforced at two points. 3. *Exclude time-dependent behavior from oracle scope* — guts L1 (fills, cooldowns, funded gates are all time-coupled); rejected.
- **DECISION:** Option 2, designed in full: trait definitions, injection-point map for every RS-0.6 category-(b) group, mulberry32+Box-Muller bit-exact port requirement, clippy `disallowed-methods` wall for `SystemTime::now`/`Utc::now` outside the clock module, golden-side ID-normalization recommendation (operator ruling queued at RS-1.3), double-run hash proof semantics. Doc: `docs/plans/rs-clock-rng-injection-design.md`.
- **Status:** SHIPPED (design; consumed by RS-1.1 and RS-1.3). Closes with RS-1.3's double-run determinism proof green in CI.

### P-139 · RS-UP-1 / RS-0.6 — Determinism audit of the TS replay path: **replay is NOT decision-deterministic as-is** (timer-driven decisions + wall-anchored pacing + wall-bucketed candles); fully salvageable at driver level — SHIPPED (2026-07-24, `docs/plans/rs-determinism-audit.md`)
- **PROBLEM (evidenced):** the entire RS-UP-1 oracle strategy assumes reproducible TS replay (risk R3, "the plan dies if not"). Measured 2026-07-24 @ `332f99e`: 223 `Date.now`/`Math.random`/`new Date` sites in non-test main/shared code + 58 timer sites. Decision path is timer-driven: `autonomous-trader.ts:131` decides on `setTimeout(intervalMs)`; sim fills land via `setTimeout(slip.delayMs)`/`setTimeout(50)` (`trading-engine.ts:1249,1254`); funded overlay ticks 60 s (`:742`); replay cursor is wall-anchored (`replay-source.ts:387,392`) and the pump is a wall interval (`:324`); candles bucket by wall clock (`live-candle-buffer.ts:41,111,183`). Two wall-clock replay runs of the same corpus produce different goldens.
- **SOLUTIONS considered:** (see audit §Verdict) per-site edits vs driver-level fake-time harness vs scope exclusion — analysis in the doc.
- **DECISION:** 100% classification shipped: ≈96 (a) deterministic/irrelevant · ≈118 (b) needs-injection (neutralized wholesale by driver-level fake clock) · 9 (c) genuine races (collapsed by fake timers). Silver lining measured: the codebase already carries seams (`order-manager ctx.nowMs`, `self-eval`/`wire-feed` `deps.now`, `market-hours` date params, seeded mulberry32 sim RNG — "no Math.random in the simulator path" by design). Remedy handed to RS-0.7 (P-140). **Plan tripwire verdict: R3 fired in week 1 exactly as designed, cost = a document, not a quarter.**
- **Coordination note (P-090 class, live occurrence):** a sibling work-layer session ran concurrently with this one (2026-07-24 evening) and holds an uncommitted ledger claim labeled "P-137" (leaf-component jsdom coverage) in the `mc4` working tree — colliding with the committed P-137 (RS-0.3, PR #75, merged). Committed beats uncommitted: the sibling entry renumbers (P-141+) at its close. Its 8 untracked leaf-component test files + spec were left untouched by this session.
- **Gates:** read-only audit — no subject code changed; TS four-gate bar unaffected (CI-arbitrated on the docs PR).
- **Status:** SHIPPED. GO condition per plan spec: operator reads the audit findings (queued for M0 exit ceremony).

### P-138 · RS-UP-1 / RS-0.4 — additive Rust CI: six-gate bar jobs (ubuntu fast + windows arbiter) — SHIPPED (2026-07-24, ships with the RS-0.2/0.3 scaffold PR from `feat/rs-0.2-workspace-scaffold`)
- **PROBLEM:** the scaffold needs CI enforcement from its first merge (RS-L6: a gate bar that exists only locally is not a bar), and D-016 names windows-latest the merge-blocking arbiter.
- **SOLUTIONS considered:** 1. *Third-party Rust CI actions* (dtolnay/rust-toolchain, taiki-e/install-action, Swatinem/rust-cache) — convenient, but adds three new supply-chain trust roots to a repo whose only current actions are `actions/*` official. 2. *Inline pinned-release installs, sha256-verified, no new actions* — more workflow code, zero new trust roots; toolchain resolution stays owned by `rust-toolchain.toml` (single source, no version drift between workflow and file). 3. *cargo install in CI* — compiles deny/machete every run, ~8–10 min tax per job.
- **DECISION:** Option 2. Two additive jobs (`rust-gates-linux` advisory-fast, `rust-gates-windows` arbiter) each running the exact Appendix F six-gate sequence; cargo-deny **0.20.2** + cargo-machete **v0.9.2** pinned by URL + sha256 (both platforms); TS `Gates` job byte-untouched. Deferred as follow-ups (ledger on observation): path-filtering (jobs currently run on TS-only PRs too — accepted cold-start cost, empty workspace builds < ~3 min), caching. **Remaining to close RS-0.4:** [O] operator wires `Rust gates (windows arbiter)` into the `main-protection` required checks; [A] prove-the-gate-can-fail scratch PR (P-097 law) after this workflow lands on master.
- **Status:** SHIPPED pending merge; evidence (local six-gate run + CI links) in the PR body.

### P-137 · RS-UP-1 / RS-0.3 — Rust policy files: pinned toolchain, deny.toml, clippy denials, forbid(unsafe_code) — SHIPPED (2026-07-24, ships with the RS-0.2 scaffold PR)
- **PROBLEM:** plan 0.D forbids `unsafe`, unjustified deps, and f32 in engine math; those walls must exist as *machine-enforced config* before any port lands, not as prose.
- **SOLUTIONS considered:** 1. *Per-crate lint attributes only* — visible in source but forgettable on new crates. 2. *Workspace-level `[workspace.lints]` + `clippy.toml` + `deny.toml` + pinned `rust-toolchain.toml`, inherited by every crate via `[lints] workspace = true`* — one authority, new crates inherit by construction. Chosen.
- **DECISION:** Option 2, shipped: `rust-toolchain.toml` pins **1.97.1** (+rustfmt/clippy components); `[workspace.lints.rust] unsafe_code = "forbid"` (and `#![forbid(unsafe_code)]` restated in every crate root for legibility); `clippy::unwrap_used`/`expect_used`/`disallowed_types` = deny with `clippy.toml` disallowing `f32` (Appendix B.1 — engine math is f64-only; local `#[allow]` with justification is the escape hatch, reviewed); `deny.toml` = license allowlist, yanked-deny, wildcard-ban, crates-io-only sources (no git deps, plan 0.D).
- **Status:** SHIPPED pending merge; gate evidence in the PR body.

### P-136 · RS-UP-1 / RS-0.2 — Cargo workspace scaffold: 14 crates stubbed, compiles empty, six-gate green — SHIPPED (2026-07-24, PR from `feat/rs-0.2-workspace-scaffold`)
- **PROBLEM:** RS-UP-1 Phase 0 needs the workspace skeleton (`apps/satex-engine-rs`, §2.3 crate map) merged early so every later port lands additively on `master` (RS-L7 — no long-lived rewrite branch).
- **SOLUTIONS considered:** 1. *Scaffold crates lazily as each phase starts* — smaller initial diff, but the ⚠️ perimeter crates and the D-013 lane exclusions wouldn't exist as enforceable paths until Phase 8, and every early PR would churn workspace membership. 2. *Scaffold all 14 crates now, empty-but-compiling, with perimeter marking in the crate descriptions/docs* — the whole §2.3 topology becomes real, reviewable, and CI-covered on day 0. Chosen (matches RS-0.2 as written).
- **DECISION:** Option 2. Workspace at `apps/satex-engine-rs`: resolver 3, edition 2024, `rust-version 1.97`; 14 crates per §2.3 (`satex-core/-data/-broker/-broker-alpaca/-risk⚠️/-exec⚠️/-intel/-backtest/-persist/-engine/-health/-ipc/-shell⚠️*/-parity`), each `publish = false`, `[lints] workspace = true`, crate-level docs naming its plan task; Tauri **2.11.5** + tauri-plugin-updater **2.10.1** pinned in `[workspace.dependencies]` as recorded pins (nothing consumes them until RS-9.1/9.5 — zero dep-tree cost now, D-012 rows due at consumption). Environment scar recorded: on operator hardware rustc's MSVC detection fails under the agent-harness/Git-Bash PATH (coreutils `link.exe` shadow → all test binaries fail to link); fix = run cargo via `vcvars64.bat` (AGENTS.md §Rust-lane note). Toolchain 1.97.1 installed this session (P-135(b)).
- **Status:** SHIPPED pending merge; local six-gate results + CI links in the PR body. Next: RS-0.6 determinism audit (P-139 candidate).

### P-135 · RS-UP-1 ADOPTED — the Rust engine rewrite ultraplan (v1.0.0 draft → v1.0.1) enters execution; Phase 0 foundation begins — IN-PROGRESS (2026-07-24, operator ratification on record)
- **PROBLEM (evidenced):** The ~74 KB rewrite plan `2026-07-22-satex-rs-rewrite-ultraplan.md` sat untracked at repo root in DRAFT FOR OPERATOR ADOPTION status — zero tracked references, decision register unratified (D-001–D-016 Proposed, D-017 OPEN) — and its projected coordinates had drifted per its own honesty axiom: authored against `master @ e145dd5` / ledger head P-129 (projected claim P-130); re-measured at adoption 2026-07-24: `master @ d5d0922` (5 intervening commits — P-131 docblock, P-094 VERIFIED flip, P-132 gitignore, P-133 test wave ×2 — none touching the ported engine surface) / ledger head P-134 → claim renumbered **P-135**. Environment re-verify per plan 0.F: Rust toolchain ABSENT (cargo/rustc/rustup not found); MSVC Build Tools 2022 (VC.Tools.x86.x64) present.
- **SOLUTIONS considered:** 1. *Adopt with corrected baseline + ratify the register's recommendation column wholesale* — fastest to execution; every decision remains individually re-openable by ledger entry with evidence. 2. *Walk D-002–D-016 individually with the operator before adopting* — higher ceremony; no recommendation was contested and each is still reversible under option 1. 3. *Hold adoption pending plan revision* — no known defect justified a hold; the baseline drift is header-level, not structural.
- **DECISION + rationale (2026-07-24, interactive session, operator present):** Option 1. Operator explicitly ratified three gates: **(a)** adopt RS-UP-1 + ratify D-001–D-016 as recommended (D-017 stays OPEN pending RS-1.2 corpus sizing); **(b)** install the Rust toolchain now — installed + verified this session: **rustc 1.97.1 / cargo 1.97.1, `stable-x86_64-pc-windows-msvc`** (exact pin re-verified at RS-0.2 per RS-L5); **(c)** RS-0.1 worktree bootstrap delegated to the agent under explicit operator consent ([O]-task delegation recorded here) — `git worktree add --detach ../mc4-rust master`. Adoption pack shipped in this commit: plan → `docs/plans/` with header corrections (v1.0.1), this entry, `ARCHITECTURE.md` §1 note, CHANGELOG entry, Scar-Tissue Port Ledger seed at `docs/plans/rs-scar-ledger.md` (Appendix B.4). Also this session: stale 0-byte `.git/index.lock` cleared from the Windows host per §2.9 documented recovery — P-134 finding 2's lock was host-deletable interactively (the EPERM applies to the sandbox mount only); P-134 finding 1 (Cowork scheduling outage) remains OPEN, untouched.
- **Gates:** docs-only commit, zero app-code contact (the plan's own RS-L7: the Rust workspace grows additively; the shipping terminal is unaffected). Pre-commit typecheck+lint enforced at commit; full four-gate bar CI-arbitrated on the PR.
- **Status:** IN-PROGRESS — this entry is the plan's standing anchor. Per claim protocol 0.C, each RS-x.y task claims its own P-number referencing `RS-UP-1 / RS-x.y`. Next free: P-136 (RS-0.2 workspace scaffold candidate).

### P-134 · Unattended scheduling health — the dawn planner (`satex-psd-daily`, 05:00) has produced NO handoff for three consecutive days (2026-07-22 · -23 · -24), and a fresh stale `.git/index.lock` (P-099/P-125 class) recurred within four days of P-125's "durable" auto-clean fix — OPEN (operator-domain: scheduling infra is out of this session's reach)
- **PROBLEM (evidenced, two related operational findings, this run @ 2026-07-24 21:39 CDT — an off-nominal work-layer fire, ~15.6h past the nominal 06:00):**
  1. **Dawn-planner no-fire, 3-day pattern.** `ls Vault/Daily/` shows the last agent-handoff is `2026-07-21-agent-handoff.md`; there is NO `2026-07-22`, `-23`, or `-24` handoff. Under the P-106 two-file contract the work-layer boots off the dawn handoff; three straight days of fallback (this run included) means the 05:00 planner is not running. Corroborated: the 2026-07-23 work-layer report already flagged "confirm `satex-psd-daily` actually fired on 2026-07-22 (no handoff file appeared that day)," and its housekeeping item 7 found `schtasks /query` + a session `CronList` both show NO SATEX/Claude-named task — the schedule lives in a Cowork `/schedule` routine outside any agent session's visibility.
  2. **Stale-lock recurrence despite P-125.** `git status` this run emitted `warning: unable to unlink '.git/index.lock': Operation not permitted` (EPERM from the sandbox mount) — a fresh stale lock. P-125 (SHIPPED 2026-07-20) root-caused this class to Defender/Search-indexer racing git's unlink and shipped a "durable two-layer fix" (scanner-exclusion script + auto-clean hook); the 2026-07-23 session had to manually clear an index.lock again; and it is back on 2026-07-24. Git READS still work (log/status/diff all succeeded), so this run was not blocked — but the P-125 remediation is demonstrably not holding across sessions.
- **SOLUTIONS considered:** 1. *Operator-only fixes (both are out of session reach)* — (a) verify/re-arm the `satex-psd-daily` Cowork routine so 05:00 handoffs resume; (b) run `scripts/git-unlock.ps1` (or delete `.git/index.lock` in Explorer) and confirm the P-125 scanner-exclusion actually applied on this machine. Zero agent write; correct owner for scheduling + machine-local locks. 2. *Widen P-125's auto-clean hook to also fire on session boot from inside the agent* — but the hook is a git-side mechanism and the sandbox cannot unlink the lock (EPERM proven this run), so an agent-side clean is not reliably possible here; also risks racing a real git op. 3. *Fold finding 1 into P-090 (scheduled-agent coordination) and finding 2 into P-125 as recurrence notes* — keeps history together but buries a live 3-day operational outage inside older DECIDED/SHIPPED entries where the next dawn (if it ever fires) won't surface it.
- **DECISION + rationale (2026-07-24, work-layer fallback run): Option 1 — record as a standalone OPEN operator-domain entry; take no agent write on either finding.** Both root causes sit outside any agent session's authority: the schedule is a Cowork routine not visible to `schtasks`/`CronList`, and the lock is unlinkable only from real hardware (EPERM here). This is exactly the P-090/§2.4-adjacent "don't touch live scheduling infrastructure on unconfirmed assumptions" boundary the 2026-07-23 session respected. A standalone OPEN entry (over Option 3's burial) keeps the 3-day outage visible at the ledger head until the operator acts. No unilateral fix, no guess.
- **Gates:** N/A — documentation-only, no app code touched this run. Independent audit sweep run alongside (see report): unbounded-spread (P-093/P-108 class), degenerate-slice, empty-catch/error-swallow, timer-leak, and NUL/`\r\r` corruption classes all CLEAN across `src/` and the 18 files in `HEAD~6..HEAD` — no new code defect found; full-repo `npm run typecheck` exit 0 (both tsconfigs). Lint/test/knip: CI-arbitrated (no subject changed; last full-green on this master per the 2026-07-23 report).
- **Status:** OPEN (operator action required on both sub-findings). Ledgered by the 2026-07-24 work-layer fallback run; report at `Vault/Daily/2026-07-24-work-layer.md`.

### P-131 · MAY-TACTICS drawdown veto latches for the whole session — the veto-LIFT branch (`tactics.ts:174-178`) is near-unreachable, and the docblock's promised "until drawdown recovers" recovery cannot happen via recorded trades — RESOLVED (2026-07-22, Option 2: the latch is an accepted fail-safe; docblock corrected to describe it truthfully)
- **PROBLEM:** `TacticsEngine.refresh()` (`tactics.ts:169-179`) sets the drawdown veto when `m.maxDrawdown > MAX_DRAWDOWN_VETO` (0.06) and clears it in the branch `else if (this.vetoActive && m.maxDrawdown < MAX_DRAWDOWN_VETO * 0.5)` (< 0.03). But `metrics().maxDrawdown` (`tactics.ts:187-193`) is `maxDd = Math.max(maxDd, (peak - equity) / Math.max(1, peak || 1000))` — a **running max over the entire retained `history`**, recomputed fresh each call. The only runtime mutations of `history` are `push` + front-`shift` at the 500 cap (`recordOutcome`, `tactics.ts:88-89`); a tree-wide grep for `history = []` / `.length =` / `splice` / `pop` / any reset returned **zero** hits. So `maxDrawdown` is monotonic non-decreasing under append: once a >6% drawdown enters the buffer, recording recovery trades can **never** pull it back under the 3% lift threshold. The lift branch can therefore only fire when the trough-defining trades age out of the 500-cap FIFO (500+ subsequent trades) or history is wiped by the boot-time store-version reset. (It is also doubly gated: `vetoActive` initializes `false`, so the constructor's first `refresh()` can only ACTIVATE — the lift branch needs a prior activation.) Net effect: **a tripped MAY-TACTICS drawdown veto stays latched for the rest of the session regardless of subsequent performance.** This contradicts the subject's own docblock (`tactics.ts:7-9`): "block all entries until **drawdown recovers** or daily session resets" — there is no drawdown-recovery path via trades, and no daily-session veto-reset call exists for the history. A doc/impl divergence on a §2.4 perimeter interlock.
- **SOLUTIONS considered:** 1. *Windowed / since-marker drawdown* — compute the drawdown veto over a recent window (or since a reset marker) so it can genuinely recover as the docblock promises. Perimeter behavior change; needs its own blueprint + human sign-off. 2. *Accept the latch as intentional and fix the docblock* — a breached circuit-breaker staying shut for the session is defensible; correct `tactics.ts:7-9` to say the veto persists until the drawdown trades age out of the 500-buffer or history resets. Docs-only, zero behavior change. 3. *Explicit daily veto reset* — clear `vetoActive` (not history) at a day boundary / boot so a new session starts clean. Behavior change on the interlock.
- **DECISION + rationale (2026-07-22, operator-directed via process of elimination): Option 2 — accept the latch, correct the docblock.** The decisive fact: the MAY-TACTICS gate is a **restriction** (an active veto *blocks* entries), so a latched veto **fails safe** — it can only ever prevent trades, never permit an unwanted one. That makes this a doc/impl divergence, not a safety hole. **Option 1 (windowed drawdown)** and **Option 3 (daily reset)** both make the veto *easier to clear* — i.e. they WEAKEN a risk control (the less-safe direction) on speculation, with no evidence the latch causes real harm in the paper-only phase, and each is a §2.4 perimeter behavior change; both eliminated. **Option 2** keeps the safe/conservative behavior, aligns with the priority stack (P2 risk ≫ P5 profit) and the learning-phase mandate, and closes the real hazard — a misleading comment in a safety interlock that a future maintainer could "fix" toward. Windowed-drawdown recovery stays a future option ONLY if evidence later shows the latch is too aggressive, and only via a fresh RISK-TOUCH blueprint + sign-off.
- **Gates:** `tactics.ts` change is **comment-only** (docblock), zero logic bytes — verified: every changed line is a ` *` comment line, no code line added/removed. Behavior unchanged, still pinned by the `tactics.test.ts` veto-stickiness test (P-094 pass). Reachability evidence verified against `tactics.ts:88-89` (only push/shift), `:169-179` (refresh branches), `:187-193` (running-max drawdown), and a zero-hit grep for history-reset paths across `src/`. Full four-gate bar + CI re-arbitrate at merge.
- **Status:** RESOLVED (2026-07-22, Option 2) — `tactics.ts` docblock corrected to describe the running-max latch truthfully; no behavior change. Branch `docs/p131-tactics-veto-latch`.

### P-133 · Chart-subtree jsdom-harness coverage — wave 2 of the P-129 map, closing the program: `export.ts` + `useChartOpts.ts` + all 7 remaining view `.tsx` (OrderFlowTape, MultiTFOverlay, CanvasOverlay, CrosshairReadout, DrawingLayer, DrawingToolbar, FootprintLayer); 118 characterization tests added, all 9 subjects byte-unchanged — SHIPPED (2026-07-23, unstaged for operator review)
- **PROBLEM:** P-130 (wave 1) closed `useIPC.ts` + `drawing-renderer.ts` and left the P-129 map's remaining 9 targets for the next session: `chart/export.ts` (207 L, LWC-screenshot + WebGL + 2D-overlay canvas compositing → IPC hand-off, plus a pure drawing→SVG serialiser), `hooks/useChartOpts.ts` (59 L, localStorage-backed display-option hook), and 7 DOM/canvas-coupled view components (`flow/OrderFlowTape.tsx` 198 L — rAF-throttled ring-buffer tape with raw-innerHTML rendering; `MultiTFOverlay.tsx` 267 L — a second real `lightweight-charts` instance + `NavController` + crosshair-sync bridge; `overlay/CanvasOverlay.tsx` 162 L — `forwardRef`/`useImperativeHandle` DPR-aware canvas with a self-rescheduling rAF loop + `ResizeObserver`; `overlay/CrosshairReadout.tsx` 170 L — binary-search nearest-candle OHLCV readout; `drawing/DrawingLayer.tsx` 79 L — frustum-culled rAF drawing-render dispatch; `drawing/DrawingToolbar.tsx` 105 L — real DOM tool/undo/redo/clear buttons over `drawingStore`; `webgl/FootprintLayer.tsx` 63 L — `WebGLRenderer` lifecycle bridge). All 9 shipped with zero regression pin; several carry the exact defect classes prior waves fixed elsewhere in this codebase (e.g. `CanvasOverlay`'s DPR scaling mirrors the P-039/P-040/P-093 degenerate-input lineage; `OrderFlowTape`'s 500-print ring buffer is the same cap-and-drop-oldest shape as `tradesStore` P-127) with no test proving the guard holds.
- **SOLUTIONS considered:** 1. *Force all 9 files into a single unattended pass.* Rejected — P-129 explicitly warned against a rushed jsdom suite; the 3 heaviest subjects (`CanvasOverlay`, `OrderFlowTape`, `MultiTFOverlay`) each needed their own controllable double (cancel-aware rAF queue, `ResizeObserver` stub, or a full `lightweight-charts`/`NavController` module mock) and deserved individual verification, not a batch rubber-stamp. 2. *Extend the wave-1 hand-rolled harness pattern (no new deps) per subject, verifying each file's gates independently before moving to the next.* **Chosen** — same zero-dependency discipline as P-130, but session-paced: write, run shuffled ×2, typecheck, lint, only then move on. 3. *Stop after export.ts + useChartOpts.ts (a same-size batch to wave 1) and leave the 7 views for a further session.* Rejected this time — operator explicitly directed a full push through the remaining map tonight; all 9 completed within gate-verified budget, so the program closes in one sitting rather than fragmenting further.
- **DECISION + rationale:** (2). Off-perimeter throughout (renderer UI + pure canvas/chart-library wiring; no order/risk/kill/arm/credential/update-feed path). Per-file harness notes: `export.test.ts` (21 tests, jsdom) — `electron`'s `ipcRenderer` mocked at the module boundary (matches `auto-update.test.ts`); the offscreen composite canvas faked via a `document.createElement` spy scoped to the `'canvas'` tag only; pins the LWC-screenshot-failure / null-2d-context / blob-null / IPC-reject false paths, webgl/overlay `drawImage` failure swallowing, the timestamped-filename contract, and the SVG serialiser's hline/line/annotation rendering + raster-only-kind exclusion + XML escaping. `useChartOpts.test.tsx` (9 tests) — real `localStorage` (jsdom-native), pins default/merge/malformed-JSON fallback, the mount-time save, the quota-exceeded swallow, and `setOpt`'s stable `useCallback` identity. `FootprintLayer.test.tsx` (9 tests) — `WebGLRenderer` module-mocked (constructible regular-function factory, not an arrow — arrows cannot be `new`-ed); pins create/destroy pairing across `enabled`/containerRef-identity changes and the MVP tint paint callback's exact GL call sequence. `DrawingLayer.test.tsx` (11 tests) — `drawing-renderer` module-mocked to isolate wiring from the already-pinned (P-130) pixel math; the REAL `drawingInView` frustum-cull runs un-mocked against a real `drawingStore`; a cancel-aware rAF stub proves a dep-change before flush cancels the stale frame (only the fresh one commits). `DrawingToolbar.test.tsx` (10 tests) — real DOM buttons + the REAL `drawingStore` (no mocking, matches the `useIPC.test.tsx` real-store-spy precedent); real `.click()` round-trips through `aria-pressed` and the per-symbol `undoSymbol` gate. `CrosshairReadout.test.tsx` (16 tests) — cancel-aware rAF stub; pins the no-time/non-finite/empty-candles clear branches, `time:0` not misread as "left the chart", the nearest-candle binary search (exact + between-candles), positive/negative bar styling, and a rapid second move cancelling the first's pending commit. `CanvasOverlay.test.tsx` (12 tests) — `HTMLCanvasElement.prototype.getContext` stubbed (the real `<canvas>` DOM node stays real so refs/`ResizeObserver.observe` behave normally, unlike `export.test.ts`'s node-substitution approach); pins DPR-aware sizing, `ResizeObserver`-driven resync, the self-rescheduling paint loop, the "always call the LATEST `onDraw` without re-running the mount effect" stable-closure pattern, and the imperative `redraw()` handle. `OrderFlowTape.test.tsx` (10 tests) — pins the 500-print FIFO cap, newest-first raw-HTML rows colored/dimmed by side+provenance, the dirty-flag rAF economy (a flush with no new trades does not touch `innerHTML`), and a genuine, real, harmless characterization: the header's ref-based print count lags one render behind the buffer (it reads `printBufRef.current.length` during render, before that render's own trades-sync effect has run). `MultiTFOverlay.test.tsx` (20 tests, closes the program) — both `lightweight-charts` (`createChart`/`CandlestickSeries`) and `./nav/NavController` module-mocked; pins secondary-chart creation sized to `height-32`, dark/light `layout` colors, the no-remount candle-refresh effect, the primary→secondary crosshair-sync price extraction (`.close ?? .value ?? 0`), the `ResizeObserver→chart.resize` bridge, the shared-Y toggle (incl. the real, harmless characterization that toggling Sync-Y on with no primary visible range yet leaves `autoScale` untouched until the primary range becomes available), and the full four-part cleanup (unsubscribe, `NavController.destroy`, `ResizeObserver.disconnect`, `chart.remove`). All 9 subjects byte-unchanged (`git diff --stat` empty across every listed file).
- **DIVERGENCE / notes:** (a) a `vi.fn().mockImplementation()` factory that must be called with `new` (WebGLRenderer, NavController mocks) needs a `function` implementation, not an arrow — arrows are not constructible and throw `TypeError: … is not a constructor`; both mocks fixed to regular functions. (b) `vi.mock('electron'/'lightweight-charts'/…)` factories create the inner `vi.fn()`s ONCE per test *file*; every suite with a module-level mock clears call history in `beforeEach` (`mockClear()`/`mockReset()`) — omitting this leaked call counts across tests in three files during drafting (export.ts's `ipcRenderer.invoke`, FootprintLayer's `WebGLRenderer`, MultiTFOverlay's `createChart`), caught immediately by the shuffled double-run. (c) jsdom performs no layout: `CanvasOverlay`'s paint loop reads `canvas.clientWidth/clientHeight` (not the container's), so the test pins those directly on the `<canvas>` node rather than relying on jsdom to compute them from the CSS width/height `syncSize()` sets. (d) `OrderFlowTape`'s time-format assertions compute the expected `toLocaleTimeString` string in-test rather than hardcoding a UTC string — the sandbox runs CDT, so a hardcoded "00:00:00" for epoch 0 failed until made timezone-relative, matching how the subject itself computes it. (e) two real, harmless behaviors were found and *characterized, not fixed* (correctness-precedes-optimization, §2.7; both off-perimeter and inert): `MultiTFOverlay` calls `series.setData` twice on the very first mount (the mount effect and the `[candles]`-dep refresh effect both fire on initial commit with identical payloads) and does not apply `autoScale:false` when Sync-Y is toggled on before the primary chart has any visible range (self-corrects the next time the effect re-fires with a real range) — neither harms behavior; both are ledgered here so a future session doesn't "fix" wave-2 assumptions in these components under automation.
- **Gates (2026-07-23, sandbox Node 22.x, against the mounted repo):** every new file **shuffled ×2** order-independent (`--sequence.shuffle`, all exit 0): `export.test.ts` 21/21, `useChartOpts.test.tsx` 9/9, `FootprintLayer.test.tsx` 9/9, `DrawingLayer.test.tsx` 11/11, `DrawingToolbar.test.tsx` 10/10, `CrosshairReadout.test.tsx` 16/16, `CanvasOverlay.test.tsx` 12/12, `OrderFlowTape.test.tsx` 10/10, `MultiTFOverlay.test.tsx` 20/20 — **118/118 new tests**. Full-project `npm run typecheck` **exit 0** (both tsconfigs, all 9 new files included) · full-project `npm run lint` **exit 0, 0 warnings** (real green, not scoped) · full-project `npm test` **158/159 files, 2052/2090 tests** (was 149/150 files, 1934/1972 at session baseline — delta is exactly the 118 new tests; the same 38 pre-existing `persistence.test.ts` failures, the documented sandbox better-sqlite3 Electron-ABI mismatch, green in CI after `npm rebuild`) · full-project vitest **re-run shuffled, identical counts** · `npm run knip` **exit 0** (21 unused-export / 30 unused-type warnings, all pre-existing `chart-indicators`/`chart/**` noise per the P-094 precedent, none from this change, none of the 9 new files listed). All 9 subjects confirmed byte-unchanged via `git diff --stat` (empty).
- **Status:** SHIPPED (2026-07-23) — 9 new test files (`src/renderer/chart/export.test.ts`, `src/renderer/hooks/useChartOpts.test.tsx`, `src/renderer/chart/webgl/FootprintLayer.test.tsx`, `src/renderer/chart/drawing/DrawingLayer.test.tsx`, `src/renderer/chart/drawing/DrawingToolbar.test.tsx`, `src/renderer/chart/overlay/CrosshairReadout.test.tsx`, `src/renderer/chart/overlay/CanvasOverlay.test.tsx`, `src/renderer/chart/flow/OrderFlowTape.test.tsx`, `src/renderer/chart/MultiTFOverlay.test.tsx`), all **unstaged** for operator review. **The P-129 jsdom-harness coverage program is now COMPLETE** — every chart-subtree target mapped in P-129 (drawing-renderer, useIPC, export, useChartOpts, and all 7 views) is pinned across P-130 + this entry. CHANGELOG: no entry (test-only, no app-behavior change; P-123/P-128/P-130 precedent).
...
- **Status:** DECIDED (2026-07-21) — survey only, zero code change; map recorded for the next dawn. **Wave 1 (useIPC + drawing-renderer) SHIPPED 2026-07-22 as P-130**; **wave 2 (export.ts, useChartOpts.ts + all 7 view .tsx) SHIPPED 2026-07-23 as P-133 — the survey's entire map is now closed, program COMPLETE.**
...
### P-094 · Six main-process services carry zero test coverage; one (`live-mode.ts`) is the live-capital arming interlock itself — VERIFIED (2026-07-22; all coverage complete — four safe autonomous picks + the two human-gated interlocks; live-mode+tactics merged to master `1a70e94` via PR #69, CI Gates green)
...
- **Update 2026-07-22 (operator-directed, Opus 4.8, human-gated perimeter) — CLOSES P-094:** the human-gated remainder (`live-mode.ts` + `tactics.ts`) is SHIPPED. Process honored CONSTITUTION §0.3/§2.4 (even *adding tests* to interlock code is human-gated): operator authorized a blueprint-first pass → ultraplan `docs/superpowers/specs/2026-07-22-live-mode-tactics-coverage-ultraplan.md` drafted, then given a max-effort operator review that source-verified every claim and **surfaced finding F-1 → P-131** (the drawdown-veto latch), then approved. Added `src/main/services/live-mode.test.ts` (NEW, **20 tests**): getLiveModeStatus paper-endpoint override (arming flag forced false on a paper URL — the safe direction), disable-always-allowed under a hostile ctx, all three enable interlocks incl. the **strict-`<` daily-loss boundary** (exactly-at-limit is NOT blocked, pinned loudly) and the **inclusive 50k notional cap** boundary (50000 ok, 50001 refused), guard **source-order** (kill-switch reason wins over a simultaneous loss breach), enable-success round-trip with fresh `updatedAt`, absent/malformed/partial/write-fail persistence (never a silent false-live), a `getLiveModeStatus` object-freshness pin (it crosses the IPC boundary — P-061/P-074 class), and the **`isLive()`-vs-`getLiveModeStatus` armed-vs-effective divergence** four engine gates depend on. Extended `src/main/services/tactics.test.ts` (**+6, 12→18**): `recordOutcome` append+persist, 500-cap **FIFO** eviction, **mid-session** veto activation (vs T4.3's boot-only), **veto stickiness** (pins P-131), and post-graduation **win-rate + expectancy** clause drift on an already-armed engine. **Correction to this entry's original framing:** `tactics.ts` was NOT zero-coverage — **P-121** (PR #63, merged `81b115a`, 2026-07-20) already shipped 12 tests for the graduation machinery; this pass is a targeted gap-fill, not a from-scratch suite. Both subjects **byte-unchanged** (`git diff` empty). **Gates (operator hardware, Node 24.15):** typecheck node+web exit 0 · eslint `src tests` exit 0 · knip exit 0 (new test files not flagged; the 52 unused-export warnings are pre-existing `chart/**` noise) · vitest **1934 pass / 38 fail** — all 38 failures are `persistence.test.ts`'s documented better-sqlite3 Electron-ABI local false-fail (green in CI after `npm rebuild`); the 38 new live-mode+tactics tests all pass, standalone and co-run (order-independent). CHANGELOG: no entry (test-only, no app-behavior change; P-123/P-128 precedent). Branch `chore/p094-live-mode-tactics-coverage`; **PR held for operator merge sign-off — not auto-merged** (P-121 precedent). Flip VERIFIED after operator adopts + CI green.
  - **VERIFIED 2026-07-22:** operator signed off; **PR #69 rebase-merged to master `1a70e94`** (linear on `c966d1c`, no merge commit), CI `Gates (typecheck, lint, knip, tests)` green (4m49s). Post-merge re-validation on synced master: `live-mode.test.ts` + `tactics.test.ts` 38/38 pass. P-094 CLOSED. Follow-on finding **P-131** (drawdown-veto latch) remains OPEN for separate operator disposition — not part of this coverage scope.
