...
- **Status:** SHIPPED (corrected) on `feat/boot-intro-sequence` — 058df50 (film sequence, superseded) + the correction commit. Operator: `git push` (fast-forward on the already-pushed branch) → PR → CI → merge.
